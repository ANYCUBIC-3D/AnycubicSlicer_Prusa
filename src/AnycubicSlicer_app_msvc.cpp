...
// http://download.qt.io/development_releases/prebuilt/llvmpipe/windows/
    if (load_mesa) {
        ...
        if (!res) {
            MessageBox(nullptr, L"AnycubicSlicer was unable to automatically switch to MESA OpenGL library\nPlease, try to run the application using the '--sw-renderer' option.\n",
                L"AnycubicSlicer Warning", MB_OK);
            return -1;
        }
        else {
...
//	printf("Loading Slic3r library: %S\n", path_to_slic3r);
    ...
    if (hInstance_Slic3r == nullptr) {
        printf("AnycubicSlicer.dll was not loaded\n");
        return -1;
    }
    ...
    if (slic3r_main == nullptr) {
        printf("could not locate the function slic3r_main in AnycubicSlicer.dll\n");
        return -1;
    }
