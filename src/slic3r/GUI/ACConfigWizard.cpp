...
#include "ACConfigWizard_private.hpp"
    ...
    for ( auto dir : dir_list) {
        ...
        for (const auto &dir_entry : boost::filesystem::directory_iterator(dir.first)) {
            if (Slic3r::is_ini_file(dir_entry)) {
                ...
                boost::asio::post(pool, [&res, file, dir, id, &lock]() {
                    Bundle bundle;
                    if (!bundle.load(file, dir.second))
                        return;
                    char val = 0;
                    while (true) {
                        if (lock.compare_exchange_strong(val, 1)) {
                            res.emplace(std::move(id), std::move(bundle));
                            lock.store(0);
                            break;
                        }
...
Bundle& BundleMap::prusa_bundle()
{
    ...
    if (it == end()) {
        throw Slic3r::RuntimeError("ACConfigWizard: Internal error in BundleMap: PRUSA_BUNDLE not loaded");
    }
...
const std::string PrinterPicker::PRINTER_PLACEHOLDER = "ACEmpty";

PrinterPicker::PrinterPicker(wxWindow *parent, const VendorProfile &vendor, wxString title, int max_cols, const AppConfig &appconfig, const ModelFilter &filter)
    : wxPanel(parent)
    , vendor_id(vendor.id)
    , width(0)
{
    ...
    for (const auto &model : models) {
        ...
        if (!found) {
            ...
            imgName = Slic3r::var(PRINTER_PLACEHOLDER);
        }

        const auto model_id = model.id;

        if (model.variants.size()) {
            ...
            cbox->SetHoverIcon(imgName);
            cbox->setSizeValid(false, true);

            cbox->Bind(wxEVT_CHECKBOX, [this, cbox](wxCommandEvent &event) {
                on_checkbox(cbox, cbox->GetChecked());
            });
    ...
    const size_t cols = std::min(max_cols, activedModelsNum);

    auto *printer_grid = new wxFlexGridSizer(cols, 20, 20);
    printer_grid->SetFlexibleDirection(wxVERTICAL | wxHORIZONTAL);

    if (activedModelsNum > 0) 
    {
    ...
    wxSize minSize = printer_grid->CalcMin();
    this->SetMinSize(minSize);

    SetSizerAndFit(printer_grid);
}

PrinterPicker::PrinterPicker(wxWindow *parent, const VendorProfile &vendor, wxString title, int max_cols, const AppConfig &appconfig)
...
void PrinterPicker::select_all(bool select, bool alternates)
{
    ...
    for (const auto &cb : cboxes_alt) {
        if (cb->GetChecked() != alternates) {
            cb->SetChecked(alternates);
...
{
    if (i < cboxes.size() && cboxes[i]->GetChecked() != select) {
        cboxes[i]->SetChecked(select);
...
ConfigWizardPage::ConfigWizardPage(ACConfigWizard *parent, wxString title, wxString shortname, unsigned indent)
...
{
    const bool data_empty = run_reason == ACConfigWizard::RR_DATA_EMPTY;
    ...
    , install(false)   // only used for 3rd party vendors
{
    ...
    parent->setButtonVisiable(ACConfigWizard::BT_ALL, false);
    parent->setButtonVisiable(ACConfigWizard::BT_NEXT | ACConfigWizard::BT_FINISH , true);
}
...
void PagePrinters::set_run_reason(ACConfigWizard::RunReason run_reason)
{
    if (is_primary_printer_page
        && (run_reason == ACConfigWizard::RR_DATA_EMPTY || run_reason == ACConfigWizard::RR_DATA_LEGACY)
...
ACButton* createTitle(wxWindow* parent, const wxString& title)
{
    wxString markIconName = "title-circle";

    ACButton* title_item = new ACButton(parent, title, markIconName, markIconName, markIconName, wxNO_BORDER|ACButton::AC_ALIGN_LEFT, wxSize(10,10));
    title_item->SetCanFocus(false);
    title_item->SetEnable(false);
    title_item->SetButtonType(ACButton::AC_BUTTON_LABEL);
    title_item->SetSpacing(10);
    title_item->SetPaddingSize(wxSize(0,0));

    return title_item;
}

PageMaterials::PageMaterials(ACConfigWizard *parent, Materials *materials, wxString title, wxString shortname, wxString list1name)
    ...
    , materials(materials)
{
    ...
    pagePanel ->SetCornerRadius(14);
    pagePanel ->SetBackgroundColour(AC_COLOR_PANEL_BG);
    pagePanel ->SetSizerAndFit(panelSizer);

    wxBoxSizer *leftSizer  = new wxBoxSizer(wxVERTICAL);
    wxBoxSizer *rightSizer = new wxBoxSizer(wxVERTICAL);

    ACStaticBox* leftPanel  = new ACStaticBox(pagePanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxNO_BORDER);
    ACStaticBox* rightPanel = new ACStaticBox(pagePanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxNO_BORDER);
    leftPanel ->SetCornerRadius(11);
    rightPanel->SetCornerRadius(11);
    leftPanel ->SetBackgroundColour(AC_COLOR_WHITE);
    rightPanel->SetBackgroundColour(AC_COLOR_WHITE);
    leftPanel ->SetSizerAndFit(leftSizer);
    rightPanel->SetSizerAndFit(rightSizer);

    panelSizer->Add(leftPanel, 1, wxEXPAND|wxLEFT|wxTOP|wxBOTTOM, 12);
    panelSizer->AddSpacer(12);
    panelSizer->Add(rightPanel, 1, wxEXPAND|wxTOP|wxBOTTOM|wxRIGHT, 12);

    
    wxScrolled<ACStaticBox>* rightScroll = new wxScrolled<ACStaticBox>(rightPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL | wxBORDER_NONE);
    wxBoxSizer* m_rightScroll_sizer = new wxBoxSizer(wxVERTICAL);
    rightScroll->SetSizer(m_rightScroll_sizer);
    rightScroll->SetScrollbars(1, 20, 1, 2);
    rightScroll->SetBackgroundColour(AC_COLOR_WHITE);
    rightScroll->SetCornerRadius(11);


    ACButton* title_printer = createTitle(leftPanel, _L("Printer"));
    ACButton* title_type    = createTitle(leftPanel, _L("Type"   ));
    ACButton* title_vendor  = createTitle(leftPanel, _L("Vendor" ));
    ACButton* title_profile = createTitle(rightPanel, _L("Profile"));

	list_printer = new ACCheckListBox(leftPanel  , _L("Printer"), ACButton::AC_BUTTON_CHECK_L0, ACButton::CHECKSTYLE_ON, 2);
    list_type    = new ACCheckListBox(leftPanel  , _L("Type"   ), ACButton::AC_BUTTON_CHECK_L1, ACButton::CHECKSTYLE_ON_MARK, 4);
    list_vendor  = new ACCheckListBox(leftPanel  , _L("Vendor" ), ACButton::AC_BUTTON_CHECK_L1, ACButton::CHECKSTYLE_ON_MARK, 3);
    list_profile = new ACCheckListBox(rightScroll, _L("Profile"), ACButton::AC_BUTTON_LV3, ACButton::CHECKSTYLE_ON_BOX, 1);
        
    m_rightScroll_sizer->Add(list_profile, 1, wxEXPAND);

    list_printer->setMultSel(true);
    list_profile->setMultSel(true);

    leftSizer->AddSpacer(12);

    leftSizer->Add(title_printer, 0, wxEXPAND|wxLEFT|wxRIGHT, 12);
    leftSizer->AddSpacer(14);
    leftSizer->Add(list_printer, 0, wxEXPAND|wxLEFT|wxRIGHT, 12);
    
    leftSizer->AddSpacer(20);

    leftSizer->Add(title_type, 0, wxEXPAND|wxLEFT|wxRIGHT, 12);
    leftSizer->AddSpacer(14);
    leftSizer->Add(list_type   , 0, wxEXPAND|wxLEFT|wxRIGHT, 12);

    leftSizer->AddSpacer(20);

    leftSizer->Add(title_vendor, 0, wxEXPAND|wxLEFT|wxRIGHT, 12);
    leftSizer->AddSpacer(14);
    leftSizer->Add(list_vendor , 0, wxEXPAND|wxLEFT|wxRIGHT, 12);
    
    leftSizer->AddStretchSpacer();

    rightSizer->AddSpacer(12);
    rightSizer->Add(title_profile, 0, wxEXPAND|wxLEFT|wxRIGHT, 12);
    rightSizer->AddSpacer(14);
    rightSizer->Add(rightScroll, 1, wxEXPAND|wxLEFT|wxRIGHT, 12);
    rightSizer->AddSpacer(12);
    ...
    parent->setButtonVisiable(ACConfigWizard::BT_ALL, false);
    parent->setButtonVisiable(ACConfigWizard::BT_PREV | ACConfigWizard::BT_FINISH , true);
    ...
    list_profile->Bind(wxEVT_LISTBOX, [this](wxCommandEvent& evt) { 
    ...
    append(pagePanel, 1, wxEXPAND | wxALL, 14);

    wxSize pageMin = page_sizer->CalcMin();
    wxSize leftMin = leftSizer->CalcMin();
    leftPanel->SetMinSize(leftMin);

    this->SetMinSize(wxSize(std::max(pageMin.x, 960), std::max(pageMin.y, 680)));
}
...
void PageMaterials::reload_presets()
{
    ...
    //if (materials->technology == T_FFF && app_config->get("no_templates") != "1")
...
void PageMaterials::update_lists(int sel_type, int sel_vendor, int last_selected_printer/* = -1*/)
{
	...
	std::vector<int> sel_printers;
	int sel_printers_count = list_printer->GetSelections(sel_printers);

    bool templates_available = false; // list_printer->GetCount() > 1 && list_printer->get_data(1) == TEMPLATES;
    ...
    if (!are_equal(sel_printers, sel_printers_prev)) {
        ...
        if (sel_printers_count > 0 && last_selected_printer == 0) {
        ...
        //else 
        //if (materials->technology == T_FFF && templates_available && sel_printers_count > 0 && last_selected_printer == 1) {
		...
		if (sel_printers_count != 0 && sel_type != wxNOT_FOUND && sel_vendor != wxNOT_FOUND) {
            ...
            for (int i = 0; i < sel_printers_count; i++) {
				...
				materials->filter_presets(printer, printer_name, type, vendor, [this, &to_list](const Preset* p) {
                    ...
                    list_profile->SetCheck(cur_i, checked || was_checked);
...
void PageMaterials::sort_list_data(ACCheckListBox* list, bool add_All_item, bool material_type_ordering)
{
// get data from list
// sort data
// first should be <all>
...
// in alphabetical order
    ...
    for (int i = 0 ; i < list->GetCount(); ++i) {
        ...
        //    continue;
        //}
        if (!material_type_ordering && data.find("Anycubic") != std::string::npos)
...
void PageMaterials::sort_list_data(ACCheckListBox* list, const std::vector<ProfilePrintData>& data)
{
    // sort data
    // then prusa profiles
    // then the rest
    ...
    for (const auto& item : data) {
        ...
        if (name.find("Anycubic") != std::string::npos)
    ...
    for (size_t i = 0; i < prusa_profiles.size(); ++i) {
        ...
        list->SetCheck(i, prusa_profiles[i].checked);
    }
    for (size_t i = 0; i < other_profiles.size(); ++i) {
        ...
        list->SetCheck(i + prusa_profiles.size(), other_profiles[i].checked);
    }
}
...
void PageMaterials::select_all(bool select)
{
    ...
    for (unsigned i = 0; i < list_profile->GetCount(); i++) {
        ...
        if (current != select) {
            list_profile->SetCheck(i, select);
            select_material(i);
        }
    }
}
...
PageCustom::PageCustom(ACConfigWizard *parent)
...
PageUpdate::PageUpdate(ACConfigWizard *parent)
...
PageDownloader::PageDownloader(ACConfigWizard* parent)
...
bool DownloaderUtils::Worker::perform_register(const std::string& path_override/* = {}*/)
{
    ...
    wxRegKey key_first(wxRegKey::HKCU, "Software\\Classes\\AnycubicSlicer");
    wxRegKey key_full(wxRegKey::HKCU, "Software\\Classes\\AnycubicSlicer\\shell\\open\\command");
    if (!key_first.Exists()) {
    ...
    // The url will always trigger opening of Anycubic Slicer and we have to check that user has allowed it. (GUI_App::MacOpenURL is the triggered method)
#else
...
PageReloadFromDisk::PageReloadFromDisk(ACConfigWizard* parent)
...
PageVendors::PageVendors(ACConfigWizard *parent)
...
PageFirmware::PageFirmware(ACConfigWizard *parent)
...
PageBedShape::PageBedShape(ACConfigWizard *parent)
...
PageBuildVolume::PageBuildVolume(ACConfigWizard* parent)
...
PageDiameters::PageDiameters(ACConfigWizard *parent)
...
PageTemperatures::PageTemperatures(ACConfigWizard *parent)
...
ConfigWizardIndex::ConfigWizardIndex(wxWindow *parent)
    : wxPanel(parent)
    //, bg(ScalableBitmap(parent, "logo_192px_transparent.png", 192))
...
void ACConfigWizard::priv::load_pages()
{
    ...
    //    // Filaments & Materials
        ...
        //const AppConfig* app_config = wxGetApp().app_config;
        //if (!any_fff_selected && (custom_printer_selected || custom_printer_in_bundle) && (app_config->get("no_templates") != "1")) {
...
void ACConfigWizard::priv::init_dialog_size()
{
    wxWindowUpdateLocker freeze_guard(q);
    (void)freeze_guard;
    ...
    //wxSize bestSize = q->GetBestSize();
    float titleHeight = m_topbar->GetMinSize().GetHeight();
    float btSizerHeight = btnsizer->GetMinSize().GetHeight();
    ...
    if (! app_config->legacy_datadir()) {
        ...
        for (auto &dir_entry : boost::filesystem::directory_iterator(printer_dir))
            if (Slic3r::is_ini_file(dir_entry)) {
                ...
                appconfig_new.set_variant("Anycubic", model, variant, true);
            }
    }
...
void ACConfigWizard::priv::add_page(ConfigWizardPage *page)
{
    //const int proportion = (page->shortname == _L("Filaments")) || (page->shortname == _L("SLA Materials")) ? 1 : 0;
    h_page_sizer->Add(page, 1, wxEXPAND);
    all_pages.push_back(page);
}

void ACConfigWizard::priv::enable_next(bool enable)
{
    btn_next->Enable(enable);
    btn_finish->Enable(enable);
}

void ACConfigWizard::priv::set_start_page(ACConfigWizard::StartPage start_page)
{
    switch (start_page) {
        case ACConfigWizard::SP_PRINTERS: 
            index->go_to(page_fff); 
            btn_next->SetFocus();
            break;
        case ACConfigWizard::SP_FILAMENTS:
            index->go_to(page_filaments);
            btn_finish->SetFocus();
            break;
        case ACConfigWizard::SP_MATERIALS:
...
void ACConfigWizard::priv::create_3rdparty_pages()
{
...
void ACConfigWizard::priv::set_run_reason(RunReason run_reason)
{
...
void ACConfigWizard::priv::update_materials(Technology technology)
{
...
void ACConfigWizard::priv::on_custom_setup(const bool custom_wanted)
{
	custom_printer_selected = custom_wanted;
    load_pages();
}

void ACConfigWizard::priv::on_printer_pick(PagePrinters *page, const PrinterPickerEvent &evt)
{
...
void ACConfigWizard::priv::select_default_materials_for_printer_model(const VendorProfile::PrinterModel &printer_model, Technology technology)
{
...
void ACConfigWizard::priv::select_default_materials_for_printer_models(Technology technology, const std::set<const VendorProfile::PrinterModel*> &printer_models)
{
    PageMaterials     *page_materials    = technology & T_FFF ? page_filaments : page_sla_materials;
...
bool ACConfigWizard::priv::on_bnt_finish()
{
    wxBusyCursor wait;
...
bool ACConfigWizard::priv::check_and_install_missing_materials(Technology technology, const std::string &only_for_model_id)
{
...
bool ACConfigWizard::priv::apply_config(AppConfig *app_config, PresetBundle *preset_bundle, const PresetUpdater *updater, bool& apply_keeped_changes)
{
    wxString header, caption = _L("Configuration is edited in ACConfigWizard");
    ...
    if (preferred_pt = get_preferred_printer_technology("Anycubic", bundles.prusa_bundle());
    ...
    // BOOST_LOG_TRIVIAL(debug) << "ACConfigWizard::priv::apply_config integrate_desktop" << page_welcome->integrate_desktop()  << "
    // perform_registration_linux " << page_downloader->m_downloader->get_perform_registration_linux(); if (page_welcome->integrate_desktop())
    ...
    switch (run_reason) {
        case ACConfigWizard::RR_DATA_EMPTY:
            snapshot = false;
            break;
        case ACConfigWizard::RR_DATA_LEGACY:
            snapshot = true;
            break;
        case ACConfigWizard::RR_DATA_INCOMPAT:
            // In this case snapshot has already been taken by
            // PresetUpdater with the appropriate reason
            snapshot = false;
            break;
        case ACConfigWizard::RR_USER:
    ...
    // Prusa printers are considered first, then 3rd party.
    if (preferred_model = get_preferred_printer_model("Anycubic", bundles.prusa_bundle(), preferred_variant);
    ...
    //std::string first_added_sla_material;
...
void ACConfigWizard::priv::update_presets_in_config(const std::string& section, const std::string& alias_key, bool add)
{
...
bool ACConfigWizard::priv::check_fff_selected()
{
...
bool ACConfigWizard::priv::check_sla_selected()
{
    bool ret = false;
...
{
    BOOST_LOG_TRIVIAL(info) << boost::format("Running ACConfigWizard, reason: %1%, start_page: %2%") % reason % start_page;
...
const wxString& ACConfigWizard::name(const bool from_menu/* = false*/)
{
...
void ACConfigWizard::on_dpi_changed(const wxRect &suggested_rect)
{
    p->index->msw_rescale();
...
void ACConfigWizard::on_sys_color_changed()
{
    wxGetApp().UpdateDlgDarkUI(this);
    Refresh();
}

void ACConfigWizard::setButtonVisiable(int btID, bool visiable)
{
    if (btID & BT_PREV ) {
        p->btn_prev->Show(visiable);
    } 
    
    if (btID & BT_NEXT) {
        p->btn_next->Show(visiable);
    } 
    
    if (btID & BT_FINISH) {
        p->btn_finish->Show(visiable);
    }  
