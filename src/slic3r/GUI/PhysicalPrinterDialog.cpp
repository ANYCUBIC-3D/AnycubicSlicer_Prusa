    ...
    m_parent(parent)
{
    ...
    m_delete_preset_btn = new ScalableButton(parent, wxID_ANY, "ACEmpty");
    ...
    m_printer("", wxGetApp().preset_bundle->physical_printers.default_config())
{
    ...
    m_add_preset_btn = new ScalableButton(this, wxID_ANY, "ACEmpty");
...
void PhysicalPrinterDialog::build_printhost_settings(ConfigOptionsGroup* m_optgroup)
{
    ...
    {
        auto sizer = create_sizer_with_btn(parent, &m_printhost_browse_btn, "ACEmpty", _L("Browse") + " " + dots);
    ...
    auto print_host_test = [=](wxWindow* parent) {
        auto sizer = create_sizer_with_btn(parent, &m_printhost_test_btn, "ACEmpty", _L("Test"));
    ...
    auto print_host_printers = [this, create_sizer_with_btn](wxWindow* parent) {
        ...
        auto sizer = create_sizer_with_btn(parent, &m_printhost_port_browse_btn, "ACEmpty", _(L("Refresh Printers")));
    ...
    if (Http::ca_file_supported()) {
        ...
        auto printhost_cafile_browse = [=](wxWindow* parent) {
            auto sizer = create_sizer_with_btn(parent, &m_printhost_cafile_browse_btn, "ACEmpty", _L("Browse") + " " + dots);
...
void PhysicalPrinterDialog::update(bool printer_change)
{
    ...
    if (tech == ptFFF) {
        ...
        if (Field* printhost_field = m_optgroup->get_field("print_host"); printhost_field) {
            if (wxTextCtrl* temp = dynamic_cast<wxTextCtrl*>(printhost_field->getWindow()); temp && temp->GetValue() == L"https://connect.xxxxxxx.com") {
            ...
            if (opt->value == htPrusaConnect) { // automatically show default prusaconnect address
                if (Field* printhost_field = m_optgroup->get_field("print_host"); printhost_field) {
                    if (wxTextCtrl* temp = dynamic_cast<wxTextCtrl*>(printhost_field->getWindow()); temp && temp->GetValue().IsEmpty()) {
                        temp->SetValue(L"https://connect.xxxxxxx.com");
                    }
                }
            }
        }
...
void PhysicalPrinterDialog::update_host_type(bool printer_change)
{
    ...
    for (PresetForPrinter* prstft : m_presets) {
        ...
        if (Preset* preset = wxGetApp().preset_bundle->printers.find_preset(preset_name)) {
            ...
            if (preset->vendor) {
                if (preset->vendor->name == "Anycubic") {
    ...
    for (PresetForPrinter* prstft : m_presets) {
        ...
        std::string model_id = preset->config.opt_string("printer_model");
        if (preset->vendor && preset->vendor->name != "Anycubic") {
            connect.supported = false;
            break;
        }
        if (preset->vendor && preset->vendor->name != "Anycubic") {
