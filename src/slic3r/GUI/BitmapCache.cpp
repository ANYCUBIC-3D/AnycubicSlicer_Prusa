                                      ...
                                      const bool dark_mode, const std::string& new_color /*= ""*/)
{
    ...
    std::string pathName;
    if (boost::filesystem::exists(bitmap_name + ".svg")) {
        pathName = bitmap_name + ".svg";
    } else {
        pathName = Slic3r::var(bitmap_name + ".svg");
    }

    std::string str;
    nsvgGetDataFromFileWithReplace(pathName.c_str(), str, replaces);
    if (str.empty())
        return nullptr;
...
wxBitmapBundle* BitmapCache::from_png(const std::string& bitmap_name, unsigned width, unsigned height)
{
    ...
    std::string pathName;
    if (boost::filesystem::exists(bitmap_name + ".png")) {
        pathName = bitmap_name + ".png";
    } else {
        pathName = Slic3r::var(bitmap_name + ".png");
    }

    wxImage image;
    if (!image.LoadFile(from_u8( pathName ), wxBITMAP_TYPE_PNG) ||
        image.GetWidth() == 0 || image.GetHeight() == 0)
        return nullptr;
...
{
    std::string bitmap_key = (color.empty() ? "ACEmpty" : color) + "-h" + std::to_string(height) + "-w" + std::to_string(width) + (dark_mode ? "-dm" : "");
