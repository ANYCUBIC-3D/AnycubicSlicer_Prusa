...
MsgDialog::MsgDialog(wxWindow *parent, const wxString &title, const wxString &headline, long style, wxBitmap bitmap)
    ...
    : wxDialog( wxGetApp().mainframe->m_settings_dialog.IsVisible() ?  &(wxGetApp().mainframe->m_settings_dialog) : parent ? parent : dynamic_cast<wxWindow *>(wxGetApp().mainframe),
               ...
               title,
               wxDefaultPosition,
               wxSize(47 * wxGetApp().em_unit(), 20 * wxGetApp().em_unit()),
               wxNO_BORDER)
...
{
    if (ACButton *btn = get_button(btn_id)) {
        btn->SetLabel(label);
        if (set_focus)
            btn->SetFocus();
    }
...
ACButton *MsgDialog::add_button(wxWindowID btn_id, bool set_focus /*= false*/, const wxString &label /* = wxString()*/)
{
    ...
    ACButton *btn = new ACButton(this, showlable);
    btn->SetMinSize(wxSize(PRESRT_BTN_WIDTH, PRESRT_BTN_HEIGHT));
    btn->SetPaddingSize(wxSize(10, 0));
    btn->SetCornerRadius(4);
    if (btn_id == wxID_OK || btn_id == wxID_YES) {
        btn->SetButtonType(ACButton::AC_BUTTON_LV0);
    } else {
        btn->SetButtonType(ACButton::AC_BUTTON_LV1);
    }
    ...
    btn_sizer->Add(btn, 0, wxRIGHT | wxALIGN_CENTER_VERTICAL, HORIZ_SPACING);
    btn_sizer->Layout();
    btn->Bind(wxEVT_BUTTON, [this, btn_id](wxCommandEvent&) { this->EndModal(btn_id); });
    return btn;
};

ACButton *MsgDialog::get_button(wxWindowID btn_id) { return static_cast<ACButton *>(FindWindowById(btn_id, this)); }
...
void MsgDialog::apply_style(long style)
{
    ...
    /*std::string icon_name = style & wxICON_WARNING        ? "ACEmpty" :
                            ...
                            style & wxICON_QUESTION       ? "ACEmpty"    : "AnycubicSlicer";*/
    std::string icon_name = "AnycubicSlicer";
	...
	, msg(msg)
{
	...
	logo->SetBitmap(*get_bmp_bundle("logo_192px_grayscale.png", monospaced_font ? 48 : /*1*/84));
