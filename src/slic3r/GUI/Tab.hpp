...
class Page// : public wxScrolledWindow
{
	...
	wxString		m_title;
	wxString        m_iconName;
    wxString        m_hover_iconName;
    wxString        m_dis_iconName;
...
public:
    ...
    Page(wxWindow *      parent,
         const wxString &title,
         const wxString &iconName,
         const wxString &hover_iconName,
         const wxString &dis_iconName
	);
	~Page();
...
public:
	...
	wxBoxSizer*	vsizer() const { return m_page_sizer; }
	wxWindow*	parent() const { return m_parent; }
	const wxString&	title()	 const { return m_title; }
	wxString		iconName() const { return m_iconName; }
    wxString        hover_iconName() const { return m_hover_iconName; }
    wxString        dis_iconName() const { return m_dis_iconName; }
	...
	ConfigOptionsGroupShp	new_optgroup(const wxString& title, int noncommon_label_width = -1, std::function<void(t_config_option_key , boost::any, ConfigOptionsGroup* )> on_change = nullptr);
...
protected:
	...
	wxBoxSizer*			m_h_page_sizer;
	//wxTreeCtrl*			m_treectrl;
	ACListBox*			m_listbox;
	wxImageList*		m_icons;

	ACScrolledWindow*	m_page_view {nullptr};
	wxBoxSizer*			m_page_sizer {nullptr};

    //ModeSizer*			m_mode_sizer {nullptr};
	ACModeButtons*       m_mode_buttons = nullptr;
