...
//    settings category name      related bitmap name
    // ptFFF
    { L("Layers and Perimeters"), "ACEmpty"      },
    { L("Infill")               , "ACEmpty"      },
    { L("Ironing")              , "ACEmpty"     },
    { L("Fuzzy Skin")           , "ACEmpty"  },
    { L("Support material")     , "ACEmpty"     },
    { L("Speed")                , "ACEmpty"        },
    { L("Extruders")            , "ACEmpty"      },
    { L("Extrusion Width")      , "ACEmpty"      },
    { L("Wipe options")         , "ACEmpty"      },
    { L("Skirt and brim")       , "ACEmpty"  },
//  { L("Speed > Acceleration") , "ACEmpty"        },
    { L("Advanced")             , "ACEmpty"      },
    // ptSLA                    ,               
    { L("Supports")             , "ACEmpty"     },
    { L("Pad")                  , "ACEmpty"         },
...
static const constexpr std::array<std::pair<const char *, const char *>, 5> ADD_VOLUME_MENU_ITEMS = {{
    ...
    {L("Add part"),              "ACEmpty" },           // ~ModelVolumeType::MODEL_PART
    {L("Add negative volume"),   "ACEmpty" },       // ~ModelVolumeType::NEGATIVE_VOLUME
    {L("Add modifier"),          "ACEmpty"},        // ~ModelVolumeType::PARAMETER_MODIFIER
    {L("Add support blocker"),   "ACEmpty"},     // ~ModelVolumeType::SUPPORT_BLOCKER
    {L("Add support enforcer"),  "ACEmpty"},    // ~ModelVolumeType::SUPPORT_ENFORCER
}};

// Note: id accords to type of the sub-object (adding volume), so sequence of the menu items is important
static const constexpr std::array<std::pair<const char *, const char *>, 3> TEXT_VOLUME_ICONS {{
//       menu_item Name              menu_item bitmap name
        {L("Add text"),             "ACEmpty"},        // ~ModelVolumeType::MODEL_PART
        {L("Add negative text"),    "ACEmpty" },   // ~ModelVolumeType::NEGATIVE_VOLUME
        {L("Add text modifier"),    "ACEmpty"},    // ~ModelVolumeType::PARAMETER_MODIFIER
}};
...
void MenuFactory::append_menu_item_delete(wxMenu* menu)
{
    append_menu_item(menu, wxID_ANY, _L("Delete") + "\tDel", _L("Remove the selected object"),
        [](wxCommandEvent&) { plater()->remove_selected(); }, "ACEmpty", nullptr, 
...
wxMenuItem* MenuFactory::append_menu_item_layers_editing(wxMenu* menu)
{
    return append_menu_item(menu, wxID_ANY, _L("Height range Modifier"), "",
        [](wxCommandEvent&) { obj_list()->layers_editing(); }, "ACEmpty", menu,
        []() { return obj_list()->is_instance_or_object_selected(); }, m_parent);
}
...
wxMenuItem* MenuFactory::append_menu_item_settings(wxMenu* menu_)
{
    ...
    menu_item->SetBitmap(*get_bmp_bundle("ACEmpty"));
...
void MenuFactory::append_menu_item_change_extruder(wxMenu* menu)
{
    ...
    append_submenu(menu, extruder_selection_menu, wxID_ANY, name, _L("Use another extruder"),
        "ACEmpty"/* : "ACEmpty"*/, []() {return true; }, m_parent);
...
void MenuFactory::create_default_menu()
{
    ...
    append_submenu(&m_default_menu, sub_menu, wxID_ANY, _L("Add Shape"), "", "ACEmpty",
        []() {return true; }, m_parent);
}
...
void MenuFactory::create_part_menu()
{
    ...
    append_menu_item(menu, wxID_ANY, _L("Split"), _L("Split the selected object into individual parts"),
        [](wxCommandEvent&) { plater()->split_volume(); }, "ACEmpty", nullptr,
        []() { return plater()->can_split(false); }, m_parent);
...
wxMenu* MenuFactory::multi_selection_menu()
{
    ...
    if (list_model()->GetItemType(sels[0]) != itVolume) {
        ...
        if (wxGetApp().get_mode() != comSimple)
            ...
            [](wxCommandEvent&) { plater()->set_number_of_copies();    }, "ACEmpty", nullptr,
