...
//#include "ACStaticBox.hpp"
    ...
    ACButton* logo = new ACButton(this, "", "logo", "logo", "logo", wxBORDER_NONE, 60);
    ACLabel* title = new ACLabel(this, _L("Welcome to Anycubic Slicer"));
    ACLabel* info_0 = new ACLabel(this, _L("Please follow these steps to set up you Anycubic Slicer"));
    ACLabel* info_1 = new ACLabel(this, _L("This will only take a few moments"));

    ACButton* getStart = new ACButton(this, _L("Get Started"), "", "", "", wxBORDER_NONE, 40);
    ...
    vSizer->AddSpacer(5);
    vSizer->Add(lineSizer_logo, 1);
    vSizer->AddSpacer(5);
    vSizer->Add(lineSizer_Title, 1);
    vSizer->AddSpacer(5);
    vSizer->Add(lineSizer_Line0, 1);
    vSizer->Add(lineSizer_Line1, 1);
    vSizer->AddSpacer(20);
    vSizer->Add(lineSizer_next, 1);
    vSizer->AddSpacer(10);
...
bool DownloaderUtils::Worker::perform_register(const std::string& path_override/* = {}*/)
{
...
#ifdef _WIN32
    // Registry key creation for "AnycubicSlicer://" URL
    ...
    //key_full = "\"C:\\Program Files\\Prusa3D\\AnycubicSlicer\\Anycubic-slicer-console.exe\" \"%1\"";
    key_full = key_string;
#elif __APPLE__
    // Apple registers for custom url in info.plist thus it has to be already registered since build.
    // The url will always trigger opening of AnycubicSlicer and we have to check that user has allowed it. (GUI_App::MacOpenURL is the triggered method)
#else
...
//    cb_gcode = new wxCheckBox(this, wxID_ANY, _L("Associate .gcode files to AnycubicSlicer G-code Viewer"));
...
ConfigWizardIndex::ConfigWizardIndex(wxWindow *parent)
    ...
    , bullet_black(ScalableBitmap(parent, "logo.png"))
    , bullet_blue(ScalableBitmap(parent, "logo.png"))
    , bullet_white(ScalableBitmap(parent, "logo.png"))
    , item_active(NO_ITEM)
    , item_hover(NO_ITEM)
    , last_page((size_t)-1)
{
