...
void OG_CustomCtrl::CtrlLine::msw_rescale()
{
    ...
    if (draw_just_act_buttons)
        height = get_bitmap_size(get_bmp_bundle("ACEmpty"), ctrl).GetHeight();
...
void OG_CustomCtrl::CtrlLine::render(wxDC& dc, wxCoord v_pos)
{
    ...
    if (ctrl->opt_group->label_width != 0 && !label.IsEmpty()) {
        ...
        h_pos = draw_text(dc, wxPoint(h_pos, v_pos), label + ":", nullptr, ctrl->opt_group->label_width * ctrl->m_em_unit, false);
    }
...
wxCoord OG_CustomCtrl::CtrlLine::draw_mode_bmp(wxDC& dc, wxCoord v_pos)
{
    ...
    wxBitmapBundle* bmp = get_bmp_bundle("ACEmpty", wxOSX ? 10 : 12, wxGetApp().get_mode_btn_color(mode));
...
{
    wxBitmapBundle* bmp_blinking = get_bmp_bundle(is_blinking ? "ACEmpty" : "ACEmpty");
    wxCoord h_pos = pos.x;
