...
void Tab::create_preset_tab()
{
    ...
    m_tmp_panel->SetSizer(mainSizer);
    m_tmp_panel->Layout();
    ...
    add_scaled_button(panel, &m_btn_save_preset, "icon-CopyToNew-nor", wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 34); // 1 0
    // TRN "Save current Settings"
    //m_btn_save_preset->SetToolTip(from_u8((boost::format(_utf8(L("Save current %s"))) % m_title).str()));
    m_btn_save_preset->SetToolTip(_(L("Copy this preset")));
    m_btn_save_preset->setScalableButtonImg_hover("icon-CopyToNew-nor", "icon-CopyToNew-hover");

    add_scaled_button(panel, &m_btn_delete_preset, "icon-delete-nor", wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 34); // 2 0
    m_btn_delete_preset->SetToolTip(_(L("Delete this preset")));
    m_btn_delete_preset->setScalableButtonImg_hover("icon-delete-nor", "icon-delete-hover");
    // m_btn_delete_preset->Hide();
    //if (!(m_type == Preset::Type::TYPE_PRINTER))
    if (m_type == Preset::Type::TYPE_PRINTER || m_type == Preset::Type::TYPE_FILAMENT) {
        add_scaled_button(panel, &m_btn_edit_ph_printer, "icon-setting-nor", wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 34); // 3 0
        m_btn_edit_ph_printer->setScalableButtonImg_hover("icon-setting-nor", "icon-setting-hover");
    }

    //add_scaled_button(panel, &m_btn_hide_incompatible_presets, "ACEmpty", wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 34); // 4

    //add_scaled_button(panel, &m_question_btn, "icon-reset-nor", wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 34);              // 5 0
    //m_question_btn->SetToolTip(_(L("Hover the cursor over buttons to find more information \n"
    //                               "or click this button.")));

    // Bitmaps to be shown on the "Undo user changes" button next to each input field.
    add_scaled_bitmap(this, m_bmp_up_value_revert, "icon-reset-nor", 26);
    add_scaled_bitmap(this, m_bmp_white_bullet, "title-circle",0);

    // Bitmaps to be shown on the "Revert to system" aka "Lock to system" button next to each input field.
    add_scaled_bitmap(this, m_bmp_value_revert, "reset-nor", 26);
    add_scaled_bitmap(this, m_bmp_value_lock, "ACEmpty", 0);
    add_scaled_bitmap(this, m_bmp_value_unlock, "ACEmpty", 0);

    add_scaled_button(panel, &m_undo_to_sys_btn, m_bmp_white_bullet.name(), wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 34); // 6
    add_scaled_button(panel, &m_undo_btn, m_bmp_white_bullet.name(), wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 34);        // 7
    m_undo_btn->setScalableButtonImg_hover("icon-reset-nor", "icon-reset-hover");
    //m_undo_btn->Show(false);
    //add_scaled_button(panel, &m_search_btn, "ACEmpty", wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 1); // 8


    //m_search_btn->SetToolTip(format_wxstr(_L("Search in settings [%1%]"), "Ctrl+F"));

    //add_scaled_button(panel, &m_btn_compare_preset, "icon-rename-nor", wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 34); // 9
    //m_btn_compare_preset->SetToolTip(_L("Compare this preset with some another"));

    //add_scaled_button(panel, &m_btn_rename_preset, "ACEmpty", wxEmptyString, wxBU_EXACTFIT | wxNO_BORDER, 34);
    ...
    m_h_preset_sizer         = new wxBoxSizer(wxHORIZONTAL);
    mainSizer->Add(m_h_preset_sizer, 0, wxEXPAND | wxALL, 14);
    m_h_preset_sizer->Add(m_presets_choice, 0, wxALIGN_CENTER_VERTICAL);
    //m_h_preset_sizer->AddSpacer(int(10*scale_factor));


    m_h_buttons_sizer = new wxBoxSizer(wxHORIZONTAL);
    m_h_buttons_sizer->Add(m_btn_save_preset, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, int(10 * scale_factor));

    if (m_btn_edit_ph_printer) {
        //m_h_buttons_sizer->AddSpacer(int(10 * scale_factor));
        m_h_buttons_sizer->Add(m_btn_edit_ph_printer, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, int(10 * scale_factor));
    }

    //m_h_buttons_sizer->Add(m_btn_rename_preset, 0, wxALIGN_CENTER_VERTICAL);
    //m_h_buttons_sizer->AddSpacer(int(10 * scale_factor));
    m_h_buttons_sizer->Add(m_undo_btn, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, int(10 * scale_factor));

    //m_h_buttons_sizer->AddSpacer(int(10 * scale_factor));
    m_line_preset = new wxPanel(this, wxID_ANY, wxDefaultPosition, wxSize(2 * scale_factor, 14 * scale_factor));
    m_line_preset->SetBackgroundColour(AC_COLOR_FONT_GRAY);
    m_h_buttons_sizer->Add(m_line_preset, 0, wxEXPAND | wxALIGN_CENTER_VERTICAL | wxLEFT | wxTOP | wxBOTTOM, int(15 * scale_factor));

    m_h_buttons_sizer->Add(m_btn_delete_preset, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, int(10 * scale_factor));
    ...
    m_mode_buttons = new ACModeButtons(panel);
    m_h_buttons_sizer->Add(m_mode_buttons, 0, wxALIGN_CENTER_VERTICAL);

    m_h_preset_sizer->Add(m_h_buttons_sizer, 1, wxEXPAND);
    m_h_preset_sizer->AddSpacer(int(16 * scale_factor));
    ...
    // left vertical sizer
    m_h_page_sizer = new wxBoxSizer(wxHORIZONTAL);
    mainSizer->Add(m_h_page_sizer, 1, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 14);
    ...
    m_h_page_sizer->Add(m_listbox, 0, wxEXPAND);

    m_h_page_sizer->AddSpacer(14);
    ...
    ACStaticBox *pageBackgroundPanel = new ACStaticBox(page_parent);
    pageBackgroundPanel->SetCornerRadius(10);
    pageBackgroundPanel->SetBackgroundColour(AC_COLOR_PANEL_BG);
    m_h_page_sizer->Add(pageBackgroundPanel, 1, wxEXPAND);

    wxBoxSizer *pageBackgroundPanelSizer = new wxBoxSizer(wxHORIZONTAL);
    pageBackgroundPanel->SetSizer(pageBackgroundPanelSizer);

    m_page_view  = new ACScrolledWindow(pageBackgroundPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL | wxBORDER_NONE);
    m_page_sizer = new wxBoxSizer(wxVERTICAL);
    m_page_view->SetSizer(m_page_sizer);
    m_page_view->SetScrollbars(1, 20, 1, 2);
    pageBackgroundPanelSizer->Add(m_page_view, 1, wxEXPAND | wxLEFT | wxTOP | wxBOTTOM, 12);
    m_page_view->SetBackgroundColour(AC_COLOR_PANEL_BG);
    m_page_view->SetCornerRadius(14);
    ...
    //m_btn_hide_incompatible_presets->Bind(wxEVT_BUTTON, ([this](wxCommandEvent e) { toggle_show_hide_incompatible(); }));

    if (m_btn_edit_ph_printer)
        m_btn_edit_ph_printer->Bind(wxEVT_BUTTON, [this](wxCommandEvent e) {
            // m_type == Preset::Type::TYPE_PRINT || m_type == Preset::Type::TYPE_FILAMENT
            if (m_type == Preset::Type::TYPE_PRINTER) 
                wxGetApp().run_wizard(ACConfigWizard::RR_USER, ACConfigWizard::SP_PRINTERS, &wxGetApp().mainframe->m_settings_dialog);
            else
                wxGetApp().run_wizard(ACConfigWizard::RR_USER, ACConfigWizard::SP_FILAMENTS, &wxGetApp().mainframe->m_settings_dialog);
...
void Tab::add_scaled_button(wxWindow          *parent,
                            ...
                            int bmp_px_cnt)
{
    assert(icon_name.length() == 0 || icon_name.length() && bmp_px_cnt > 0);
    *btn = new ScalableButton(parent, wxID_ANY, icon_name, label, wxDefaultSize, wxDefaultPosition, style, bmp_px_cnt);
...
void Tab::add_scaled_bitmap(wxWindow *parent, ScalableBitmap &bmp, const std::string &icon_name, const int &size)
{
    bmp = ScalableBitmap(parent, icon_name, size);
    m_scaled_bitmaps.push_back(&bmp);
}
...
Slic3r::GUI::PageShp Tab::add_options_page(const wxString    &title,
                                           const std::string &iconName,
                                           const std::string &hover_iconName,
                                           const std::string &dis_iconName,
    ...
    // Index of icon in an icon list $self->{icons}.
     ...
     if (!iconName.empty()) {
        ...
        if (m_category_icon.find(title) == m_category_icon.end()) {
            ...
            m_category_icon[title] = iconName;
        }
    }
    // Initialize the page.
    PageShp page(new Page(m_page_view, title, iconName, hover_iconName, dis_iconName));
...
//#ifdef _MSW_DARK_MODE
    ...
    //if (wxSize ok_sz = wxSize(24.4 * m_em_unit, m_presets_choice->GetBestSize().y); ok_sz != m_presets_choice->GetSize()) {
    if (wxSize ok_sz = wxSize(25 * m_em_unit, 3.4 * m_em_unit); ok_sz != m_presets_choice->GetSize()) {
        m_presets_choice->SetMinSize(ok_sz);
        m_presets_choice->SetSize(ok_sz);
        m_h_preset_sizer->Layout();
        if (wxGetApp().tabs_as_menu())
    ...
    if (m_type == Preset::TYPE_PRINTER) {
        {
            auto check_bed_custom_options = [](std::vector<std::string> &keys) {
                ...
                keys.erase(std::remove_if(keys.begin(), keys.end(),
                                          [](const std::string &key) { return key == "bed_custom_texture" || key == "bed_custom_model"; }),
                           keys.end());
...
void Tab::update_changed_tree_ui()
{
    ...
    for (int i = 0; i < m_listbox->GetButtonsCount(); ++i) {
        ...
        for (auto page : m_pages) {
            ...
            if (page->title() == "Custom G-code") {
                for (auto group : page->m_optgroups) {
                    for (const auto &kvp : group->opt_map()) {
                        const std::string &opt_key = kvp.first;
                        bool               m_sys_page      = true;
                        bool               m_modified_page = false;
                        get_sys_and_mod_flags(opt_key, m_sys_page, m_modified_page);
                        if (group->getTittleCircleRest() && group->custom_ctrl != nullptr && group->getGroupBoxObj() != nullptr &&
                            group->getCircleBackButton() != nullptr) {
                            if (!m_sys_page && m_modified_page) {
                                if (!group->getCircleBackButton()->IsShown())
                                    group->getCircleBackButton()->Show();
                            } else {
                                if (group->getCircleBackButton()->IsShown())
                                    group->getCircleBackButton()->Hide();
                            }
                        }
                    }
                }
...
{   
    ...
    m_undo_btn->        SetBitmap_(m_is_modified_values ? m_bmp_up_value_revert.name(): m_bmp_white_bullet.name());
    ...
    if (m_is_modified_values && m_is_nonsys_values) {
        m_undo_btn->Show();
...
void Tab::update_mode()
{
    ...
    if (m_mode_buttons)
        m_mode_buttons->SetSwitchState((ACSwitchButton::SwitchState) m_mode);
...
void Tab::msw_rescale()
{
    ...
    m_h_page_sizer->Layout();
    for (ACButton *acButton : m_listbox->GetButtons()) {

        acButton->messureSize();
    }
    m_listbox->Rescale();
...
void Tab::sys_color_changed()
{
    ...
    wxWindowUpdateLocker noUpdates(this);
    if (m_mode_buttons)
        m_mode_buttons->sys_color_changed();
    wxGetApp().UpdateDarkUI(this);
...
static bool m_isGcodeAuto = true;

std::string Tab::checkStringIsContain(std::string checkStr,bool isinster,std::string k_str,std::string containString)
{
    bool                     isContain = false;
    std::string              insterStr = "M900 K" + k_str + " ;Filament gcode LA 1.5";
    int                      _count    = 0;
    std::vector<std::string> results;
    std::stringstream        checkStr_s(checkStr);
    std::string              line;
    while (getline(checkStr_s, line, '\n')) {
        results.push_back(line);
    }
    for (auto it = results.begin(); it != results.end(); ++it) {
        std::string subStr = it->substr(0, 4);
        if (subStr == containString) {
            isContain = true;
            break;
        }
        _count++;
    ...
    if (isinster) {
        if (!isContain) {
            results.push_back(insterStr);
        } else {
            results[_count] = insterStr;
        }
    } else {
        if (isContain) {
            results.erase(results.begin() + _count);
        }
    }

    std::stringstream resultStr_s;
    for (const auto &containStr : results) {
        resultStr_s << containStr + "\n";
    }

    return resultStr_s.str();
}

//void Tab::editStartGcodeContentEvent(double pa_default,bool enableIndex)
//{
//    if (m_isGcodeAuto) {
//        auto *_printersConfig = &m_preset_bundle->printers.get_selected_preset().config;
//
//        std::string new_startGcodeStr = checkStringIsContain(_printersConfig->opt_string("start_gcode"), enableIndex,
//                                                             (boost::format("%.2f") % pa_default).str());
//        _printersConfig->set_key_value("start_gcode", new ConfigOptionString(new_startGcodeStr));
//    }
//}

//void Tab::editEnablepaContentEvent(bool enableIndex)
//{
//
//    auto *_filamentsConfig = &m_preset_bundle->filaments.get_selected_preset().config;
//
//    _filamentsConfig->set_key_value("enable_pressure_advance", new ConfigOptionBools({enableIndex}));
//
//}

static double FilamentTypeChcekEvent(std::vector<std::string> filament_types)
{
    if (filament_types.size() > 0) {
        std::string filament_type = filament_types[0];
        if (filament_type == "PLA") {
            return 0.05;
        } else if (filament_type == "ABS") {
            return 0.04;
        } else if (filament_type == "PTEG" || filament_type == "PET" || filament_type == "PETG") {
            return 0.08;
        }
    }
    return 0.0;
...
void Tab::on_value_change(const std::string &opt_key, const boost::any &value)
{
    ...
    if (opt_key == "rect_size_x" || opt_key == "rect_size_y") {
        Vec2d rect_size(Vec2d::Zero());
        Vec2d rect_origin(Vec2d::Zero());

        try {
            rect_size = Vec2d(m_config->opt_float("rect_size_x"),
            m_config->opt_float("rect_size_y"));
        ...
        y0 -= dy;
        y1 -= dy;
        std::vector<Vec2d> shape = {Vec2d(x0, y0), Vec2d(x1, y0), Vec2d(x1, y1), Vec2d(x0, y1)};

        load_key_value("bed_shape", shape);
        // update_changed_ui();
    }

//    if (opt_key == "post_process_enable") {
//        bool enable_set = boost::any_cast<bool>(value);
//        std::string posts;
//        if (enable_set) {
//#if defined(_WIN32)
//            boost::filesystem::path temp_path(wxStandardPaths::Get().GetExecutablePath().utf8_str().data());
//            temp_path = temp_path.parent_path() / "ArcWelder.exe";
//#else
//            boost::filesystem::path temp_path(wxStandardPaths::Get().GetPluginsDir().utf8_str().data());
//            temp_path /= "ArcWelder";
//#endif
//            posts = temp_path.string();
//        }
//
//        load_key_value("post_process", posts);
//    }
    ...
    //if (is_fff ? (opt_key == "support_material" || opt_key == "support_material_auto" || opt_key == "support_material_buildplate_only") :
    //             (opt_key == "supports_enable" || opt_key == "support_tree_type" ||
    //              opt_key == get_sla_suptree_prefix(*m_config) + "support_buildplate_only" || opt_key == "support_enforcers_only"))
...
//        return; // ys_FIXME
//    bool wipe_tower_enabled = dynamic_cast<ConfigOptionBool *>((m_preset_bundle->prints.get_edited_preset().config).option("wipe_tower"))
//                                  ->value;
...
{
    return wxGetApp().mainframe->m_settings_dialog.IsShown() ? &(wxGetApp().mainframe->m_settings_dialog) : dynamic_cast<wxWindow *>(wxGetApp().mainframe);
}
...
void Tab::build_preset_description_line(ConfigOptionsGroup *optgroup)
{
    ...
    auto detach_preset_btn = [this](wxWindow *parent) {
        m_detach_preset_btn = new ScalableButton(parent, wxID_ANY, "", _L("Detach from system preset"), wxDefaultSize, wxDefaultPosition,
        ...
        btn->Bind(wxEVT_BUTTON, [this, parent](wxCommandEvent &) {
            ...
            MessageDialog dialog(dialogParent(), msg_text, _(L("Detach preset")), wxICON_WARNING | wxYES_NO | wxCANCEL);
...
void TabPrint::build()
{
    ...
    auto        page          = add_options_page(L("Layers and perimeters"), "icon-layer-nor", "icon-layer-sel", "icon-layer-nor");
    ...
    line.widget = [this](wxWindow *parent) { return description_line_widget(parent, &m_recommended_thin_wall_thickness_description_line); };
    optgroup->append_line(line);
    ...
    line            = {"", ""};
    line.full_width = 1;
    line.widget     = [this](wxWindow *parent) { return description_line_widget(parent, &m_top_bottom_shell_thickness_explanation); };
    optgroup->append_line(line);
    ...
    page          = add_options_page(L("Infill"), "icon-infill-nor", "icon-infill-sel", "icon-infill-nor");
    ...
    page = add_options_page(L("Skirt and brim"), "icon-Skirt and brim-nor", "icon-Skirt and brim-sel", "icon-Skirt and brim-nor");
    ...
    page          = add_options_page(L("Support material"), "icon-support-nor", "icon-support-sel", "icon-support-nor");
    ...
    page     = add_options_page(L("Speed"), "icon-speed-nor", "icon-speed-sel", "icon-speed-nor");
    ...
    /*page = add_options_page(L("Multiple Extruders"), "ACEmpty");
    ...
    page     = add_options_page(L("Advanced"), "icon-advance_setting-nor", "icon-advance_setting-sel", "icon-advance_setting-nor");
    ...
    optgroup->append_single_option_line("post_process_enable");
    ...
    /*    page = add_options_page(L("Output options"), "ACEmpty");
            ...
            optgroup->append_single_option_line(option);
    */
    // page = add_options_page(L("Notes"), "ACEmpty");
    ...
    // page = add_options_page(L("Dependencies"), "ACEmpty");
...
bool Tab::validate_custom_gcode(const wxString &title, const std::string &gcode)
{
    ...
    if (invalid) {
        ...
        wxString reports = format_wxstr(_L_PLURAL("The following line %s contains reserved keywords.\nPlease remove it, as it may cause "
                                                  "problems in G-code visualization and printing time estimation.",
                                                  "The following lines %s contain reserved keywords.\nPlease remove them, as they may "
        ...
        MessageDialog dialog(dialogParent(), reports, _L("Found reserved keywords in") + " " + _(title), wxICON_WARNING | wxOK);
        dialog.ShowModal();
    }
    return !invalid;
}
...
void TabFilament::update_line_with_near_label_widget(ConfigOptionsGroupShp optgroup,
...
{
    //PageShp               page     = add_options_page(L("Filament Overrides"), "ACEmpty", "ACEmpty", "ACEmpty");
    PageShp               page     = add_options_page(L("Retraction"), "icon-retraction-nor", "icon-retraction-sel", "icon-Retraction-nor");
    ...
    for (const std::string opt_key :
         {"filament_retract_length", "filament_retract_lift", "filament_retract_lift_above", "filament_retract_lift_below",
          "filament_retract_speed", "filament_deretract_speed", "filament_retract_restart_extra", "filament_retract_before_travel",
...
void TabFilament::update_filament_overrides_page()
{
    ...
    if (!m_active_page || m_active_page->title() != "Retraction")
        return;
    ...
    std::vector<std::string> opt_keys = {"filament_retract_length",        "filament_retract_lift",
                                         ...
                                         "filament_retract_layer_change",  "filament_wipe",
...
void TabFilament::update_extruder_combobox()
{
    ...
    if (extruder_cnt != m_extruders_cb->GetCount()) {
        ...
        for (size_t id = 1; id <= extruder_cnt; id++)
            m_extruders_cb->Append(format_wxstr("%1% %2%", _L("Extruder"), id), *get_bmp_bundle("ACEmpty"));
    }
...
void TabFilament::build()
{
    ...
    auto page = add_options_page(L("Filament Settings"), "icon-material_setting-nor", "icon-material_setting-sel", "icon-material_setting-nor");
    ...
    page                      = add_options_page(L("Cooling"), "icon-cooling-nor", "icon-cooling-sel", "icon-cooling-nor");
    ...
    line            = {"", ""};
    line.full_width = 1;
    line.widget     = [this](wxWindow *parent) { return description_line_widget(parent, &m_cooling_description_line); };
    optgroup->append_line(line);
    ...
    line            = {"", ""};
    line.full_width = 1;
    line.widget     = [this](wxWindow *parent) { return description_line_widget(parent, &m_volumetric_speed_description_line); };
    optgroup->append_line(line);
    ...
    */
    ...
    const int notes_field_height = 25; // 250
                                       ...
                                       /*    page = add_options_page(L("Custom G-code"), "ACEmpty");
...
void TabFilament::reloadConfig() {

    auto *_printersConfig = &m_preset_bundle->filaments.get_selected_preset().config;
    update_dirty();
    reload_config();
    update();
...
void TabFilament::toggle_options()
{
    if (!m_active_page)
        return;
    if (m_active_page->title() == "Filament Settings") {
        bool enPA = m_config->opt_bool("enable_pressure_advance", 0);
        toggle_option("pressure_advance", enPA);
    }
    ...
    if (m_active_page->title() == "Retraction")
...
void TabPrinter::build()
{
    ...
    std::string def_preset_name = "- default " + std::string(m_printer_technology == ptFFF ? "FFF" : "SLA") + " -";
    m_config                    = &m_presets->find_preset(def_preset_name)->config;
    m_printer_technology == ptFFF ? build_fff() : build_sla();
...
void TabPrinter::build_print_host_upload_group(Page *page)
{
    ...
    wxString description_line_text = _L(
        ...
        "A new Physical Printer profile is created by clicking on the \"ACEmpty\" icon right of the Printer profiles combo box, "
        "by selecting the \"Add physical printer\" item in the Printer combo box. The Physical Printer profile editor opens "
        "also when clicking on the \"ACEmpty\" icon in the Printer settings tab. The Physical Printer profiles are being stored "
        "into AnycubicSlicer/physical_printer directory.");
...
static void ac_append_float_option( std::shared_ptr<ConfigOptionsGroup> optgroup,
								const std::string& opt_key,
...
{
	ConfigOptionDef def = {opt_key, coFloat};
    def.min = 0;
    def.max = 1200;
	...
	def.tooltip = tooltip;
	def.mode = mode;
	def.set_default_value(new ConfigOptionFloat{ def_val });
...
void TabPrinter::build_fff()
{
    ...
    auto page     = add_options_page(L("General"), "icon-general-nor", "icon-general-sel", "icon-general-nor");
    //auto optgroup = page->new_optgroup(L("Size and coordinates"));
    auto optgroup = page->new_optgroup(L("Size and coordinates"));


    optgroup->append_single_option_line("rect_size_x");
    optgroup->append_single_option_line("rect_size_y");
    optgroup->append_single_option_line("max_print_height");
    optgroup->append_single_option_line("z_offset");


    optgroup = page->new_optgroup(L("Extruder"));
    int  extruder_idx = 0;
    optgroup->append_single_option_line("nozzle_diameter", "", extruder_idx);

    optgroup->m_on_change = [this, extruder_idx](const t_config_option_key &opt_key, boost::any value) {
        if (m_config->opt_bool("single_extruder_multi_material") && m_extruders_count > 1 &&
            opt_key.find_first_of("nozzle_diameter") != std::string::npos) {
            ...
            if (fabs(nozzle_diameters[extruder_idx == 0 ? 1 : 0] - new_nd) > EPSILON) {
                ...
                MessageDialog dialog(dialogParent(), msg_text, _L("Nozzle diameter"), wxICON_WARNING | wxYES_NO);
            ...
            if (opt_key == "extruders_count" || opt_key == "single_extruder_multi_material") {
                ...
                if (opt_key == "single_extruder_multi_material") { // the single_extruder_multimaterial was added to force pages
                    ...
                    if (boost::any_cast<bool>(value) && m_extruders_count > 1) {
                        SuppressBackgroundProcessingUpdate sbpu;
                        std::vector<double> nozzle_diameters = static_cast<const
    ConfigOptionFloats*>(m_config->option("nozzle_diameter"))->values; const double frst_diam = nozzle_diameters[0];
    ...
    const int gcode_field_height = 15; // 150
    const int notes_field_height = 25; // 250
    page     = add_options_page(L("Custom G-code"), "icon-Custom G-code-nor", "icon-Custom G-code-sel", "icon-Custom G-code-nor");
    optgroup = page->new_optgroup(L("Start G-code"), 0);
    optgroup->setTitleCircleRest(true);
    optgroup->circleBackList.clear();
    optgroup->setCircleBack("start_gcode");
    ...
    optgroup->setTitleCircleRest(true);
    optgroup->circleBackList.clear();
    optgroup->setCircleBack("end_gcode");
    optgroup->m_on_change = [this, &optgroup_title = optgroup->title](const t_config_option_key &opt_key, const boost::any &value) {
        validate_custom_gcode_cb(this, optgroup_title, opt_key, value);
    };
    ...
    option.opt.height     = 1.4 * gcode_field_height; // 210;
    optgroup->append_single_option_line(option);

    optgroup              = page->new_optgroup(L("Before layer change G-code"), 0);
    optgroup->setTitleCircleRest(true);
    optgroup->circleBackList.clear();
    optgroup->setCircleBack("before_layer_gcode");
    optgroup->m_on_change = [this, &optgroup_title = optgroup->title](const t_config_option_key &opt_key, const boost::any &value) {
        validate_custom_gcode_cb(this, optgroup_title, opt_key, value);
    };
    ...
    optgroup->setTitleCircleRest(true);
    optgroup->circleBackList.clear();
    optgroup->setCircleBack("layer_gcode");
    optgroup->m_on_change = [this, &optgroup_title = optgroup->title](const t_config_option_key &opt_key, const boost::any &value) {
        validate_custom_gcode_cb(this, optgroup_title, opt_key, value);
    };
    ...
    optgroup->setTitleCircleRest(true);
    optgroup->circleBackList.clear();
    optgroup->setCircleBack("color_change_gcode");
    optgroup->m_on_change = [this, &optgroup_title = optgroup->title](const t_config_option_key &opt_key, const boost::any &value) {
        validate_custom_gcode_cb(this, optgroup_title, opt_key, value);
    };
    ...
    optgroup->setTitleCircleRest(true);
    optgroup->circleBackList.clear();
    optgroup->setCircleBack("pause_print_gcode");
    optgroup->m_on_change = [this, &optgroup_title = optgroup->title](const t_config_option_key &opt_key, const boost::any &value) {
        validate_custom_gcode_cb(this, optgroup_title, opt_key, value);
    };
...
void TabPrinter::build_sla()
{
    if (!m_pages.empty())
        m_pages.resize(0);
    auto page     = add_options_page(L("General"), "ACEmpty", "ACEmpty", "ACEmpty");
    auto optgroup = page->new_optgroup(L("Size and coordinates"));
    ...
    page                  = add_options_page(L("Notes"), "ACEmpty", "ACEmpty", "ACEmpty");
    optgroup              = page->new_optgroup(L("Notes"), 0);
    option                = optgroup->get_option("printer_notes");
    option.opt.full_width = true;
    option.opt.height     = notes_field_height; // 250;
    ...
    page     = add_options_page(L("Dependencies"), "ACEmpty", "ACEmpty", "ACEmpty");
    optgroup = page->new_optgroup(L("Profile dependencies"));
...
{
    auto page = add_options_page(L("Machine limits"), "ACEmpty", "ACEmpty", "ACEmpty", true);
    ...
    auto optgroup = page->new_optgroup(L("General"));
    {
        ...
        Line line{"", ""};
        line.full_width = 1;
        line.widget     = [this](wxWindow *parent) { return description_line_widget(parent, &m_machine_limits_description_line); };
        optgroup->append_line(line);
    }
...
void TabPrinter::build_extruder_pages(size_t n_before_extruders)
{
    for (auto extruder_idx = m_extruders_count_old; extruder_idx < m_extruders_count; ++extruder_idx) {
        ...
        auto            page      = add_options_page(page_name, "ACEmpty", true);
        ...
        auto reset_to_filament_color = [this, extruder_idx](wxWindow *parent) {
            ScalableButton *btn = new ScalableButton(parent, wxID_ANY, "icon-reset-click", _L("Reset to Filament Color"), wxDefaultSize,
        ...
        line.widget                   = [this, extruder_idx](wxWindow *parent) {
            ScalableButton *btn   = new ScalableButton(parent, wxID_ANY, "", _L("Apply below setting to other extruders"), wxDefaultSize,
                                                                         wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
            auto            sizer = new wxBoxSizer(wxHORIZONTAL);
            sizer->Add(btn);
            ...
            auto has_changes = [this]() {
                ...
                for (const std::string &opt : extruder_options)
                    if (std::find(dirty_options.begin(), dirty_options.end(), opt + "#" + std::to_string(extruder_idx)) !=
                        dirty_options.end())
                        return true;
                return false;
#endif
            ...
            parent->Bind(
                wxEVT_UPDATE_UI, [this, has_changes](wxUpdateUIEvent &evt) { evt.Enable(m_extruders_count > 1 && has_changes()); },
                btn->GetId());
...
void TabPrinter::build_unregular_pages(bool from_initial_build /* = false*/)
{
    ...
    if (from_initial_build ||
        ...
        auto page     = add_options_page(L("Single extruder MM setup"), "ACEmpty", "ACEmpty", "ACEmpty", true);
...
void TabPrinter::toggle_options()
{
    ...
    if (m_active_page->title().StartsWith("Extruder ", &extruder_number) && extruder_number.ToLong(&val) && val > 0 &&
        ...
        if (use_firmware_retraction && wipe) {
            ...
            MessageDialog dialog(dialogParent(),
...
#endif
    {
        ...
        if (m_type == Slic3r::Preset::TYPE_PRINTER) {
            ...
            if (printer_technology != static_cast<TabPrinter *>(this)->m_printer_technology) {
...
#ifdef _MSW_DARK_MODE
                        if (!wxGetApp().tabs_as_menu()) {
                            std::string bmp_name = tab->type() == Slic3r::Preset::TYPE_FILAMENT     ? "ACEmpty" :
                                                   tab->type() == Slic3r::Preset::TYPE_SLA_MATERIAL ? "ACEmpty" :
                                                                                                      "ACEmpty";
                ...
                if (!wxGetApp().tabs_as_menu())
                    dynamic_cast<Notebook *>(wxGetApp().tab_panel())
                        ->SetPageImage(wxGetApp().tab_panel()->FindPage(this), printer_technology == ptFFF ? "ACEmpty" : "ACEmpty");
#endif
            }
            on_presets_changed();
...
void Tab::rebuild_page_tree()
{
    ...
    for (auto p : m_pages) {
        if (!p->get_show())
            continue;
        auto itemId = m_listbox->AppendItem(translate_category(p->title(), m_type), p->iconName(), p->hover_iconName(), p->dis_iconName());
...
void Tab::update_btns_enabling()
{
    ...
    m_line_preset->Show(m_btn_delete_preset->IsShown());
    ...
    if (m_btn_edit_ph_printer) {
        //m_btn_edit_ph_printer->SetToolTip(m_preset_bundle->physical_printers.has_selection() ? _L("Edit physical printer") :
        //                                                                                       _L("Add physical printer"));
        m_btn_edit_ph_printer->SetToolTip(m_type == Preset::TYPE_PRINTER ? _L("Edit printers") : _L("Edit filaments"));
    }
    m_h_buttons_sizer->Layout();
}
...
bool Tab::select_preset(bool index,std::string preset_name, bool delete_current /*=false*/, const std::string &last_selected_ph_printer_name /* =""*/)
{
    if (index) {
        if (wxGetApp().plater()->fff_print().calib_mode() == CalibMode::Calib_PA_Line) {
            GUI::WarningDialog dialog(wxGetApp().mainframe, _L("Pressure advance test.gcode will be closed, do you want to continue?"),
                                      _L("Warning"), wxYES | wxNO);
            dialog.Bind(wxEVT_CLOSE_WINDOW, [&dialog](wxCloseEvent &event) { dialog.EndModal(wxID_NO); });
            if (dialog.ShowModal() == wxID_NO)
                return false;

            wxGetApp().plater()->fff_print().calib_mode() = CalibMode::Calib_None;
            // clear file
            //wxGetApp().plater()->new_project(true);
            wxGetApp().plater()->get_current_canvas3D()->setOpenNewProjectPar(true);
        } else if (wxGetApp().plater()->acgcode_mode() == ACGcodeMode::ACGcode_Show) {
            wxString                lastFileName(wxGetApp().plater()->acgcode_lastFile());
            boost::filesystem::path filePath(into_u8(lastFileName));
            wxString                gcode_fileName(from_u8(filePath.filename().string()));
            GUI::WarningDialog      dialog(wxGetApp().mainframe,
                                      wxString::Format(_L("%s will be closed,Do you want to continue?"), gcode_fileName), _L("Warning"),
                                      wxYES | wxNO);
            dialog.Bind(wxEVT_CLOSE_WINDOW, [&dialog](wxCloseEvent &event) { dialog.EndModal(wxID_NO); });
            if (dialog.ShowModal() == wxID_NO)
                return false;
    ...
    if (m_presets->type() == Preset::TYPE_FILAMENT) {
        m_isGcodeAuto = true;
    } else if (m_presets->type() == Preset::TYPE_PRINTER) {
        m_isGcodeAuto = false;
    }
...
{
    if (wxGetApp().plater()->acgcode_mode() == ACGcodeMode::ACGcode_Show) {
        wxString lastFileName(wxGetApp().plater()->acgcode_lastFile());
        boost::filesystem::path filePath(into_u8(lastFileName));
        wxString                gcode_fileName(from_u8(filePath.filename().string()));
        GUI::WarningDialog dialog(wxGetApp().mainframe, wxString::Format(_L("%s will be closed,Do you want to continue?"), gcode_fileName),
                                  _L("Warning"), wxYES | wxNO);
        dialog.Bind(wxEVT_CLOSE_WINDOW, [&dialog](wxCloseEvent &event) { dialog.EndModal(wxID_NO); });
        if (dialog.ShowModal() == wxID_NO)
            return false;
...
bool Tab::may_discard_current_dirty_preset(PresetCollection *presets /*= nullptr*/, const std::string &new_printer_name /*= ""*/)
{
    ...
    UnsavedChangesDialog dlg(m_type, presets, new_printer_name, dialogParent());
...
void Tab::activate_selected_page(std::function<void()> throw_if_canceled)
{
    ...
    update_changed_ui();
    if (m_active_page->title() == "Custom G-code" && m_isGcodeAuto ) {
        reloadConfig();
    }
    if (m_active_page->title() == "Filament Settings" && !m_isGcodeAuto) {
        reloadConfig();
    }
    update_description_lines();
    toggle_options();
...
{
    // There is a bug related to Ubuntu overlay scrollbars, see https://github.com/prusa3d/PrusaSlicer/issues/898 and
    // https://github.com/prusa3d/PrusaSlicer/issues/952. The issue apparently manifests when Show()ing a window with overlay scrollbars while
    // the UI is frozen. For this reason, we will Thaw the UI prematurely on Linux. This means destroing the no_updates object prematurely.
#ifdef __linux__
...
#ifdef WIN32
            && wxGetApp().mainframe->is_active_and_shown_tab(this)
#endif // __APPLE__
...
void Tab::save_preset(std::string name /*= ""*/, bool detach)
{
    ...
    if (name.empty()) {
        SavePresetDialog dlg(dialogParent(), { m_type }, detach ? _u8L("Detached") : "", from_template);
        if (dlg.ShowModal() != wxID_OK)
            return;
        name = dlg.get_name();
        if (from_template)
...
void Tab::rename_preset()
{
    ...
    SavePresetDialog dlg(dialogParent(), m_type, msg);
    if (dlg.ShowModal() != wxID_OK)
        return;
...
void Tab::delete_preset()
{
    ...
    if (m_presets_choice->is_selected_physical_printer()) {
        ...
        msg = format_wxstr(_L("Are you sure you want to delete \"%1%\" preset from the physical printer \"%2%\"?"), current_preset.name,
                           printer.name);
    } else {
	...
	MessageDialog dlg(wxGetApp().mainframe->m_settings_dialog.IsShown() ? &(wxGetApp().mainframe->m_settings_dialog) : parent(),
		msg, title, wxYES_NO | wxNO_DEFAULT | wxICON_QUESTION);
#ifdef __WXMSW__
	dlg.SetYesBtnLabel(_L("YES"));
	dlg.SetNoBtnLabel(_L("NO"));
#else
...
//{
//    m_btn_hide_incompatible_presets->SetBitmap(*get_bmp_bundle(m_show_incompatible_presets ? "ACEmpty" : "ACEmpty"));
...
wxSizer *Tab::compatible_widget_create(wxWindow *parent, PresetDependencies &deps)
{
    ...
    deps.btn = new ScalableButton(parent, wxID_ANY, "", format_wxstr(" %s %s", _L("Set"), dots),
    ...
    deps.btn->Bind(wxEVT_BUTTON, ([this, parent, &deps](wxCommandEvent e) {
                       ...
                       wxMultiChoiceDialog dlg(dialogParent(), deps.dialog_title, deps.dialog_label, presets);
...
void SubstitutionManager::add_substitution(
    int substitution_id, const std::string &plain_pattern, const std::string &format, const std::string &params, const std::string &notes)
{
    ...
    auto del_btn = new ScalableButton(m_parent, wxID_ANY, "icon-delete-nor");
    ...
    auto add_text_editor = [substitution_id, top_sizer, this](const wxString &value, int opt_pos, int proportion) {
        auto editor = new wxTextCtrl(m_parent, wxID_ANY, value, wxDefaultPosition, wxSize(15 * m_em, wxDefaultCoord),
                                     wxTE_PROCESS_ENTER
#ifdef _WIN32
                                         | wxBORDER_SIMPLE
#endif
        ...
        editor->SetForegroundColour(AC_COLOR_BLACK);
    ...
    for (wxCheckBox *chb : std::initializer_list<wxCheckBox *>{chb_regexp, chb_case_insensitive, chb_whole_word, chb_match_single_line}) {
        ...
        chb->Bind(wxEVT_CHECKBOX,
                  [this, substitution_id, chb_regexp, chb_case_insensitive, chb_whole_word, chb_match_single_line](wxCommandEvent e) {
...
bool SubstitutionManager::is_empty_substitutions() { return m_config->option<ConfigOptionStrings>("gcode_substitutions")->values.empty(); }
...
wxSizer *TabPrint::create_manage_substitution_widget(wxWindow *parent)
{
    ...
    create_btn(&add_substitution_btn, _L("Add"), "ACEmpty");
    ...
    create_btn(&m_del_all_substitutions_btn, _L("Delete all"), "ACEmpty");
    m_del_all_substitutions_btn->Bind(wxEVT_BUTTON, [this, parent](wxCommandEvent e) {
        if (MessageDialog(parent, _L("Are you sure you want to delete all substitutions?"), wxString::FromUTF8(wxGetApp().appName()),
...
wxSizer *TabPrint::create_substitutions_widget(wxWindow *parent)
{
    wxFlexGridSizer *grid_sizer =
        new wxFlexGridSizer(2, 5, wxGetApp().em_unit()); // delete_button,  edit column contains "Find", "Replace", "Notes"
...
{
    ScalableButton *btn = new ScalableButton(parent, wxID_ANY, "", " " + _(L("Set")) + " " + dots, wxDefaultSize, wxDefaultPosition,wxBU_LEFT | wxBU_EXACTFIT);
    btn->SetFont(wxGetApp().normal_font());
    btn->SetSize(btn->GetBestSize());
    ...
    btn->Bind(wxEVT_BUTTON, ([this](wxCommandEvent e) {
                  BedShapeDialog dlg(dialogParent());
...
void TabPrinter::update_machine_limits_description(const MachineLimitsUsage usage)
{
    wxString text;
    switch (usage) {
    case MachineLimitsUsage::EmitToGCode: text = _L("Machine limits will be emitted to G-code and used to estimate print time."); break;
    case MachineLimitsUsage::TimeEstimateOnly:
        ...
        break;
    case MachineLimitsUsage::Ignore: text = _L("Machine limits are not set, therefore the print time estimate may not be accurate."); break;
...
void Tab::fill_icon_descriptions()
{
    ...
    m_icon_descriptions.emplace_back(&m_bmp_up_value_revert, L("BACK ARROW"),
...
int pageCounter = 0;
Page::Page(wxWindow *parent, const wxString &title, const wxString &iconName, const wxString &hover_iconName, const wxString &dis_iconName)
    : m_parent(parent), m_title(title), m_iconName(iconName), m_hover_iconName(hover_iconName), m_dis_iconName(dis_iconName)
{
    m_page_sizer = (wxBoxSizer *) parent->GetSizer();
    m_item_color = &wxGetApp().get_label_clr_default();
    pageCounter++;
}

Page::Page(wxWindow *parent, const wxString &title, const wxString &iconName) : Page(parent, title, iconName, iconName, iconName) {}

Page::~Page() { printf("Page Desdroyed.... %d \n", --pageCounter); }
...
void Page::activate(ConfigOptionMode mode, std::function<void()> throw_if_canceled)
{
    ...
    for (auto group : m_optgroups) {
        if (!group->activate(throw_if_canceled))
            continue;
        if (i != 0)
            m_page_sizer->AddSpacer(12);

        m_page_sizer->Add(group->sizer, 0, wxEXPAND | (group->is_legend_line() ? wxALL : wxRIGHT), 12);
...
ConfigOptionsGroupShp Page::new_optgroup(const wxString &title, int noncommon_label_width /*= -1*/, std::function<void(t_config_option_key , boost::any, ConfigOptionsGroup* )> on_change)
{
    ...
    do {
        ...
        tab = dynamic_cast<Tab *>(pWin);

    } while(tab == nullptr);

    wxASSERT(tab != nullptr);
//#ifdef __WXOSX__
//    auto tab = parent()->GetParent()->GetParent()->GetParent();// GetParent()->GetParent();
//#else
//    auto tab = parent()->GetParent()->GetParent();// GetParent();
//#endif
    optgroup->set_config_category_and_type(m_title, static_cast<Tab *>(tab)->type());
    optgroup->m_on_change = [tab, on_change, optgroup](t_config_option_key opt_key, boost::any value) {
        if (on_change)
            on_change(opt_key, value, optgroup.get());
    ...
    optgroup->rescale_extra_column_item = [](wxWindow *win) {
        ...
        ctrl->SetBitmap(reinterpret_cast<ScalableBitmap *>(ctrl->GetClientData())->get_bitmap());
    };
...
void TabSLAMaterial::build()
{
    ...
    auto page = add_options_page(L("Material"), "ACEmpty", "ACEmpty", "ACEmpty");
    ...
    page     = add_options_page(L("Material printing profile"), "ACEmpty", "ACEmpty", "ACEmpty");
...
void TabSLAPrint::build_sla_support_params(const std::vector<SamePair<std::string>> &prefixes, const Slic3r::GUI::PageShp &page)
{
    ...
    Line line{"", ""};
    line.full_width = 1;
    line.widget     = [this](wxWindow *parent) { return description_line_widget(parent, &m_support_object_elevation_description_line); };
    optgroup->append_line(line);
...
void TabSLAPrint::build()
{
    ...
    auto page = add_options_page(L("Layers and perimeters"), "ACEmpty", "ACEmpty", "ACEmpty");

    auto optgroup = page->new_optgroup(L("Layers"));
    optgroup->append_single_option_line("layer_height");
    optgroup->append_single_option_line("faded_layers");

    page = add_options_page(L("Supports"), "ACEmpty" /*"ACEmpty"*/, "ACEmpty", "ACEmpty");
    ...
    page     = add_options_page(L("Pad"), "ACEmpty", "ACEmpty", "ACEmpty");
    ...
    page     = add_options_page(L("Hollowing"), "ACEmpty", "ACEmpty", "ACEmpty");
    ...
    page     = add_options_page(L("Advanced"), "ACEmpty", "ACEmpty", "ACEmpty");
    optgroup = page->new_optgroup(L("Slicing"));
    optgroup->append_single_option_line("slice_closing_radius");
    optgroup->append_single_option_line("slicing_mode");
