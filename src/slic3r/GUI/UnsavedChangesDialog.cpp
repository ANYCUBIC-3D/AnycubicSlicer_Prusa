...
namespace Slic3r {
constexpr auto CHANGES_BTN_HEIGHT = 31;
constexpr auto CHANGES_BTN_WIDTH  = 195;
namespace GUI {
...
static const std::map<Preset::Type, std::string> type_icon_names = {
    {Preset::TYPE_PRINT,        "Icon-setting-click"           },
    {Preset::TYPE_SLA_PRINT,    "Icon-setting-click"           },
    {Preset::TYPE_FILAMENT,     "icon-material-click"         },
    {Preset::TYPE_SLA_MATERIAL, "Icon-setting-click"         },
    {Preset::TYPE_PRINTER,      "icon-machine-click"       },
};

static std::string get_icon_name(Preset::Type type, PrinterTechnology pt) {
    return pt == ptSLA && type == Preset::TYPE_PRINTER ? "ACEmpty" : type_icon_names.at(type);
}
...
static std::string orange   = "#3986ff";
...
UnsavedChangesDialog::UnsavedChangesDialog(const wxString& caption, const wxString& header, 
                                           const std::string& app_config_key, int act_buttons, wxWindow* parent)
    ...
    : DPIDialog(parent == NULL ? static_cast<wxWindow*>(wxGetApp().mainframe) : parent, wxID_ANY, caption + ": " + _L("Unsaved Changes"), wxDefaultPosition,  wxSize(84 * em_unit(), 45 * em_unit()), wxSTAY_ON_TOP|wxNO_BORDER),
...
UnsavedChangesDialog::UnsavedChangesDialog(Preset::Type type, PresetCollection* dependent_presets, const std::string& new_selected_preset, wxWindow* parent)
...
void UnsavedChangesDialog::build(Preset::Type type, PresetCollection* dependent_presets, const std::string& new_selected_preset, const wxString& header)
{
    ...
    auto add_btn = [this, buttons, btn_font, dependent_presets](ACButton** btn, int& btn_id, const std::string& icon_name, Action close_act, const wxString& label, bool process_enable = true)

    {
        //*btn = new ScalableButton(this, btn_id = NewControlId(), icon_name, label, wxDefaultSize, wxDefaultPosition, wxBORDER_DEFAULT, 24);
        *btn = new ACButton(this, label);
        (*btn)->SetMinSize(wxSize(CHANGES_BTN_WIDTH, CHANGES_BTN_HEIGHT));
        (*btn)->SetPaddingSize(wxSize(5, 5));
        (*btn)->SetCornerRadius(4);
        (*btn)->SetButtonType(ACButton::AC_BUTTON_LV0);
        buttons->Add(*btn, 1, wxLEFT|wxEXPAND, 5);
    ...
    if (ActionButtons::TRANSFER & m_buttons) {
        ...
        if (dependent_presets && switched_presets && (type == dependent_presets->type() ?
            ...
            add_btn(&m_transfer_btn, m_move_btn_id, "ACEmpty", Action::Transfer, switched_presets->get_edited_preset().name == new_selected_preset ? _L("Keep") : _L("Transfer"));
    }
    if (!m_transfer_btn && (ActionButtons::KEEP & m_buttons))
        add_btn(&m_transfer_btn, m_move_btn_id, "ACEmpty", Action::Transfer, _L("Keep"));

    { // "Don't save" / "Discard" button
        std::string btn_icon    = (ActionButtons::DONT_SAVE & m_buttons) ? "" : (dependent_presets || (ActionButtons::KEEP & m_buttons)) ? "ACEmpty" : "ACEmpty";
    ...
    if (ActionButtons::SAVE & m_buttons) 
        add_btn(&m_save_btn, m_save_btn_id, "ACEmpty", Action::Save, _L("Save"));

    //ScalableButton* cancel_btn = new ScalableButton(this, wxID_CANCEL, "ACEmpty", _L("Cancel"), wxDefaultSize, wxDefaultPosition, wxBORDER_DEFAULT, 24);
    ...
    //        wxString msg = _L("AnycubicSlicer will remember your action.") + "\n\n" + action + "\n\n" +
    //                       format_wxstr(_L("Visit \"Preferences\" and check \"%1%\"\nto be asked about unsaved changes again."), preferences_item);
    //
    //        MessageDialog dialog(nullptr, msg, _L("AnycubicSlicer: Don't ask me again"), wxOK | wxCANCEL | wxICON_INFORMATION);
...
void UnsavedChangesDialog::update_tree(Preset::Type type, PresetCollection* presets_, const std::string& new_selected_preset)
{
    ...
    for (PresetCollection* presets : presets_list)
    {
        ...
        if (type == Preset::TYPE_PRINTER && old_pt == ptFFF &&
            ...
            m_tree->Append("extruders_count", type, _L("General"), _L("Capabilities"), local_label, old_val, mod_val, new_val, category_icon_map.at(_L("General")));
        }
...
void UnsavedChangesDialog::on_dpi_changed(const wxRect& suggested_rect)
{
    ...
    const wxSize& size = wxSize(84 * em, 45 * em);
    SetMinSize(size);
...
void DiffPresetDialog::create_presets_sizer()
{
    ...
    for (auto new_type : { Preset::TYPE_PRINT, Preset::TYPE_SLA_PRINT, Preset::TYPE_FILAMENT, Preset::TYPE_SLA_MATERIAL, Preset::TYPE_PRINTER })
    {
        ...
        ScalableButton* equal_bmp = new ScalableButton(this, wxID_ANY, "ACEmpty");
        //ACButton *equal_bmp = new ACButton(this, "", "ACEmpty");
...
void DiffPresetDialog::create_buttons()
{
    ...
    // Transfer 
    m_transfer_btn = new ScalableButton(this, wxID_ANY, "ACEmpty", _L("Transfer"), wxDefaultSize, wxDefaultPosition, wxBORDER_DEFAULT, 24);
   /* m_transfer_btn = new ACButton(this,  "ACEmpty");
    m_transfer_btn->SetMinSize(wxSize(CHANGES_BTN_WIDTH, CHANGES_BTN_HEIGHT));
    m_transfer_btn->SetPaddingSize(wxSize(5, 5));
    m_transfer_btn->SetCornerRadius(4);
    m_transfer_btn->SetButtonType(ACButton::AC_BUTTON_LV0);*/
    ...
    // Save
    m_save_btn = new ScalableButton(this, wxID_ANY, "ACEmpty", _L("Save"), wxDefaultSize, wxDefaultPosition, wxBORDER_DEFAULT, 24);
    /*m_save_btn = new ACButton(this, _L("Save"));
    m_save_btn->SetMinSize(wxSize(CHANGES_BTN_WIDTH, CHANGES_BTN_HEIGHT));
    m_save_btn->SetPaddingSize(wxSize(5, 5));
    m_save_btn->SetCornerRadius(4);
    m_save_btn->SetButtonType(ACButton::AC_BUTTON_LV0);*/
    ...
    // Cancel
    m_cancel_btn = new ScalableButton(this, wxID_CANCEL, "ACEmpty", _L("Cancel"), wxDefaultSize, wxDefaultPosition, wxBORDER_DEFAULT, 24);
    /*m_cancel_btn = new ACButton(this, _L("Cancel"));
    m_cancel_btn->SetMinSize(wxSize(CHANGES_BTN_WIDTH, CHANGES_BTN_HEIGHT));
    m_cancel_btn->SetPaddingSize(wxSize(5, 5));
    m_cancel_btn->SetCornerRadius(4);
    ...
    //for (ACButton *btn : {m_transfer_btn, m_save_btn, m_cancel_btn}) {
...
void DiffPresetDialog::update_tree()
{
    ...
    for (auto preset_combos : m_preset_combos)
    {
        ...
        if (!left_preset || !right_preset) {
            ...
            preset_combos.equal_bmp->SetBitmap_(ScalableBitmap(this, "ACEmpty"));
        ...
        if (type == Preset::TYPE_PRINTER && left_pt == ptFFF &&
            ...
            m_tree->Append("extruders_count", type, _L("General"), _L("Capabilities"), local_label, left_val, right_val, "", category_icon_map.at(_L("General")));
        }
        ...
        for (const std::string& opt_key : dirty_options) {
            ...
            if (option.opt_key() != opt_key) {
                ...
                m_tree->Append(opt_key, type, _L("Undef category"), _L("Undef group"), opt_key, left_val, right_val, "", "ACEmpty");
                // When founded option isn't the correct one.
                // It can be for dirty_options: "default_print_profile", "printer_model", "printer_settings_id",
                // because of they don't exist in searcher
                continue;
