...
//	Mouse3dConnected,
//	NewPresetsAviable,
	// Notification on the start of  Anycubic Slicer, when a new  Anycubic Slicer version is published.
	// Contains a hyperlink to open a web browser pointing to the  Anycubic Slicer download location.
	NewAppAvailable,
	// Like NewAppAvailable but with text and link for alpha / bet release
	NewAlphaAvailable,
	NewBetaAvailable,
	NoNewReleaseAvailable,
	// Notification on the start of  Anycubic Slicer, when updates of system profiles are detected.
...
private:
	...
	protected:
		...
		virtual void render_hypertext(ImGuiWrapper& imgui,
		                              ...
		                              bool more = false);
        virtual void render_hypertext(
            bool _new,ImGuiWrapper &imgui, const float text_x, const float text_y, const std::string text, bool more = false);
		// Left sign could be error or warning sign
		virtual void render_left_sign(ImGuiWrapper& imgui);
		virtual void render_minimize_button(ImGuiWrapper& imgui,
			                                const float win_pos_x, const float win_pos_y);
        virtual void render_minimize_button(ImGuiWrapper &imgui, const float win_pos_x, const float win_pos_y,const bool _new);
	...
	protected:
		...
		void	render_bar(ImGuiWrapper& imgui,
							...
							const float win_pos_x, const float win_pos_y) override;
        void render_bar(
            ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new) override;
	...
	protected:
        ...
        void render_hypertext(
            bool _new,ImGuiWrapper &imgui, const float text_x, const float text_y, const std::string text, bool more = false) override;
		void       on_cancel_button();
    ...
    {NotificationType::URLNotRegistered
		...
		,  [](wxEvtHandler* evnthndlr) {
			...
			//{NotificationType::NewAppAvailable, NotificationLevel::ImportantNotificationLevel, 20,  _u8L("New vesion of  Anycubic Slicer is available.",  _u8L("Download page.") },
