...
//wxPanel specialization
template<class P> class DPIAwareEX : public P
{
public:
	DPIAwareEX(wxWindow* parent, wxWindowID id, const wxPoint& pos = wxDefaultPosition,
		const wxSize& size = wxDefaultSize, long style = wxDEFAULT_FRAME_STYLE, const wxString& name = wxFrameNameStr)
		: P(parent, id, pos, size, style, name)
	{
		int dpi = get_dpi_for_window(this);
		m_scale_factor = (float)dpi / (float)DPI_DEFAULT;
		m_prev_scale_factor = m_scale_factor;
...
typedef DPIAwareEX<wxPanel> DPIPanelEX;
