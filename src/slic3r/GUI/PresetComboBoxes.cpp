    ...
    m_preset_bundle(preset_bundle ? preset_bundle : wxGetApp().preset_bundle)
{
    ...
    /*m_bitmapCompatible   = get_bmp_bundle("ACEmpty");
    m_bitmapIncompatible = get_bmp_bundle("ACEmpty");*/

    m_bitmapCompatible   = get_empty_bmp_bundle(1,1);
    m_bitmapIncompatible = get_empty_bmp_bundle(1,1);
...
void PresetComboBox::update(std::string select_preset_name)
{
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        std::string main_icon_name = m_type == Preset::TYPE_PRINTER && preset.printer_technology() == ptSLA ? "ACEmpty" : m_main_bitmap_name;

        auto bmp = get_bmp(bitmap_key, main_icon_name, "ACEmpty", is_enabled, preset.is_compatible, preset.is_system || preset.is_default);
        assert(bmp);
    ...
    {
        set_label_marker(Append(separator(L("User presets"))));
    ...
    if (bmp_bndl == nullptr) {
        ...
        else
        {
            ...
            if (m_type == Preset::TYPE_SLA_MATERIAL)
                ...
                bmps.emplace_back(get_empty_bmp_bundle(1, 1));
            get_empty_bmp_bundle(4, 4);
            // Paint a lock at the system presets.
            bmps.emplace_back(get_empty_bmp_bundle(wide_space_icon_width, icon_height));
        }
        //bmps.emplace_back(is_system ? get_bmp_bundle("ACEmpty") : get_empty_bmp_bundle(norm_icon_width, icon_height));
        bmps.emplace_back(is_system ? get_empty_bmp_bundle(1,1) : get_empty_bmp_bundle(norm_icon_width, icon_height));
                                    ...
                                    bool is_enabled/* = true*/, bool is_compatible/* = true*/, bool is_system/* = false*/)
{
    ...
    if (bmp == nullptr) {
        ...
        bmps.emplace_back(m_type == Preset::TYPE_PRINTER ? get_empty_bmp_bundle(1,1) :
    ...
    PresetComboBox(parent, preset_type, wxSize(15 * wxGetApp().em_unit(), -1))
{
    ...
    edit_btn = new ScalableButton(parent, wxID_ANY, "ACEmpty");
...
static void run_wizard(ACConfigWizard::StartPage sp, wxWindow* parent = nullptr)
{
    wxGetApp().run_wizard(ACConfigWizard::RR_USER, sp, parent);
}
...
void PlaterPresetComboBox::OnSelect(wxCommandEvent &evt)
{
    ...
    if (marker >= LABEL_ITEM_MARKER && marker < LABEL_ITEM_MAX) {
        ...
        if (marker == LABEL_ITEM_WIZARD_PRINTERS)
            ...
            ACConfigWizard::StartPage sp = ACConfigWizard::SP_WELCOME;
            switch (marker) {
            case LABEL_ITEM_WIZARD_FILAMENTS: sp = ACConfigWizard::SP_FILAMENTS; break;
            case LABEL_ITEM_WIZARD_MATERIALS: sp = ACConfigWizard::SP_MATERIALS; break;
            default: break;
            }
            wxTheApp->CallAfter([sp, this]() { run_wizard(sp, GetParent()); });
        }
        return;
    }
...
void PlaterPresetComboBox::show_add_menu()
{
    ...
    append_menu_item(menu, wxID_ANY, _L("Add/Remove presets"), "",
        [this](wxCommandEvent&) {
            wxTheApp->CallAfter([this]() { run_wizard(ACConfigWizard::SP_PRINTERS, GetParent()); });
        }, "ACEmpty", menu, []() { return true; }, wxGetApp().plater());
...
void PlaterPresetComboBox::show_edit_menu()
{
    ...
    append_menu_item(menu, wxID_ANY, _L("Edit preset"), "",
        [this](wxCommandEvent&) { this->switch_to_tab(); }, "ACEmpty", menu, []() { return true; }, wxGetApp().plater());

#ifdef __linux__
    // To edit extruder color from the sidebar
    if (m_type == Preset::TYPE_FILAMENT) {
        append_menu_item(menu, wxID_ANY, _L("Change extruder color"), "",
            [this](wxCommandEvent&) { this->change_extruder_color(); }, "ACEmpty", menu, []() { return true; }, wxGetApp().plater());
        wxGetApp().plater()->PopupMenu(menu);
        return;
    }
#endif //__linux__
    ...
    if (this->is_selected_physical_printer()) {
        append_menu_item(menu, wxID_ANY, _L("Edit physical printer"), "",
            [this](wxCommandEvent&) { this->edit_physical_printer(); }, "ACEmpty", menu, []() { return true; }, wxGetApp().plater());

        const PhysicalPrinter& pp = m_preset_bundle->physical_printers.get_selected_printer();
        std::string host = pp.config.opt_string("print_host");
        if (!host.empty()) {
            append_menu_item(menu, wxID_ANY, _L("Open physical printer URL"), "",
                [this](wxCommandEvent&) { this->open_physical_printer_url(); }, "ACEmpty", menu, []() { return true; }, wxGetApp().plater());
        }
        

        append_menu_item(menu, wxID_ANY, _L("Delete physical printer"), "",
            [this](wxCommandEvent&) { this->del_physical_printer(); }, "ACEmpty", menu, []() { return true; }, wxGetApp().plater());
    }
    else
        append_menu_item(menu, wxID_ANY, _L("Add/Remove presets"), "",
            [this](wxCommandEvent&) {
    ...
    append_menu_item(menu, wxID_ANY, _L("Add physical printer"), "",
        [this](wxCommandEvent&) { this->add_physical_printer(); }, "ACEmpty", menu, []() { return true; }, wxGetApp().plater());
...
void PlaterPresetComboBox::update()
{
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i) 
    {
        ...
        std::string bitmap_type_name = bitmap_key = m_type == Preset::TYPE_PRINTER && preset.printer_technology() == ptSLA ? "ACEmpty" : m_main_bitmap_name;
    ...
    if (m_type == Preset::TYPE_PRINTER || m_type == Preset::TYPE_FILAMENT || m_type == Preset::TYPE_SLA_MATERIAL) {
        auto bmp = get_bmp("edit_preset_list", wide_icons, "ACEmpty");
        assert(bmp);
...
void TabPresetComboBox::OnSelect(wxCommandEvent &evt)
{
    ...
    if (marker >= LABEL_ITEM_DISABLED && marker < LABEL_ITEM_MAX) {
        ...
        if (marker == LABEL_ITEM_WIZARD_PRINTERS)
            ...
            run_wizard(ACConfigWizard::SP_PRINTERS, GetParent());
        ...
        if (!m_preset_bundle->physical_printers.empty()) {
            ...
            for (PhysicalPrinterCollection::ConstIterator it = ph_printers.begin(); it != ph_printers.end(); ++it) {
                for (const std::string& preset_name : it->get_preset_names()) {
                    ...
                    if (!preset || !preset->is_visible)
                        continue;
                    std::string main_icon_name = preset->printer_technology() == ptSLA ? "ACEmpty" : m_main_bitmap_name;
