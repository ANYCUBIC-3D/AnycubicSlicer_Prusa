...
// #endif
	...
	if (is_editor) {
		...
		append_bool_option(m_optgroup_general, "associate_3mf",
			L("Associate .3mf files to AnycubicSlicer"),
			L("If enabled, sets AnycubicSlicer as default application to open .3mf files."),
			app_config->get_bool("associate_3mf"));

		append_bool_option(m_optgroup_general, "associate_stl",
			L("Associate .stl files to AnycubicSlicer"),
			L("If enabled, sets AnycubicSlicer as default application to open .stl files."),
...
#if __APPLE__
			L("Allow just a single AnycubicSlicer instance"),
...
#else
			L("Allow just a single AnycubicSlicer instance"),
			L("If this is enabled, when starting AnycubicSlicer and another instance of the same AnycubicSlicer is already running, that instance will be reactivated instead."),
#endif
		...
		append_bool_option(m_optgroup_general, "associate_gcode",
			L("Associate .gcode files to AnycubicSlicer G-code Viewer"),
			L("If enabled, sets AnycubicSlicer G-code Viewer as default application to open .gcode files."),
	...
	append_bool_option(m_optgroup_general, "restore_win_position",
		...
		L("If enabled, AnycubicSlicer will be open at the position it was closed"),
...
/*
		append_bool_option(m_optgroup_gui, "suppress_hyperlinks",
			...
			L("If enabled, AnycubicSlicer will not open a hyperlinks in your browser."),
...
//		create_settings_font_widget();
	...
	m_title_panel = new wxPanel(this, wxID_ANY, wxDefaultPosition,wxDefaultSize, wxTAB_TRAVERSAL);
    m_title_panel->SetBackgroundColour(AC_COLOR_LIGHTBLUE);

    wxBoxSizer* title_sizer = new wxBoxSizer(wxHORIZONTAL);
    m_title_item     = new ACLabel(m_title_panel, _L("Preferences"),wxALIGN_CENTRE);
    m_title_item->SetFont(ACLabel::Body_16);
    wxString closeBtIconName      = "software_close-nor";
    wxString closeBtIconNameHover = "software_close-hover";
    m_close_button = new ACButton(m_title_panel, "", closeBtIconName,
                                  closeBtIconNameHover, closeBtIconName,
                                  wxBORDER_NONE, wxSize(30, 30));
	...
	m_title_panel->SetSizer(title_sizer);
	auto sizer = new wxBoxSizer(wxVERTICAL);
    sizer->Add(m_title_panel);
	...
	m_point = new wxPoint(0, 0);
    m_title_item->Bind(wxEVT_LEFT_DOWN, &PreferencesDialog::OnMouseLeftDown,
                       this);
    m_title_item->Bind(wxEVT_LEFT_UP, &PreferencesDialog::OnMouseLeftUp, this);
    m_title_item->Bind(wxEVT_MOTION, &PreferencesDialog::OnMouseMotion, this);

    Bind(wxEVT_LEFT_DOWN, &PreferencesDialog::OnMouseLeftDown, this);
    Bind(wxEVT_LEFT_UP, &PreferencesDialog::OnMouseLeftUp, this);
    Bind(wxEVT_MOTION, &PreferencesDialog::OnMouseMotion, this);
    Bind(wxEVT_COMMAND_BUTTON_CLICKED, &PreferencesDialog::OnClose, this,
         m_close_button->GetId());

	const wxSize min_size = wxSize(660, 660);
    int screenwidth       = wxSystemSettings::GetMetric(wxSYS_SCREEN_X, NULL);
	int screenheight      = wxSystemSettings::GetMetric(wxSYS_SCREEN_Y, NULL);
    SetPosition(wxPoint((screenwidth - min_size.x) / 2,(screenheight - min_size.y) / 2));
...
void PreferencesDialog::OnClose(wxEvent &event) { this->Close(); }

void PreferencesDialog::OnMouseLeftDown(wxMouseEvent &event)
{
    wxPoint        mouse_pos = ::wxGetMousePosition();
    wxPoint        frame_pos = this->GetScreenPosition();
    static wxPoint m_delta;
    m_delta            = mouse_pos - frame_pos;
    bool isAtbuttonPos = m_close_button != nullptr &&
                                 m_close_button->GetRect().Contains(
                                     event.GetPosition()) ?
                             true :
                             false;
    CaptureMouse();
...
#ifdef __WXMSW__
        ReleaseMouse();
        ::PostMessage((HWND) this->GetHandle(), WM_NCLBUTTONDOWN,
                      HTCAPTION, MAKELPARAM(mouse_pos.x, mouse_pos.y));
        return;
#endif //  __WXMSW__
    }
...
void PreferencesDialog::OnMouseLeftUp(wxMouseEvent &event)
{
    wxPoint mouse_pos = ::wxGetMousePosition();
...
void PreferencesDialog::OnMouseMotion(wxMouseEvent &event)
{
    wxPoint mouse_pos = ::wxGetMousePosition();
    ...
    if (event.Dragging() && event.LeftIsDown()) {
        ...
        this->Move(x - m_point->x, y - m_point->y);
        // Refresh();
    }
    event.Skip();
}
