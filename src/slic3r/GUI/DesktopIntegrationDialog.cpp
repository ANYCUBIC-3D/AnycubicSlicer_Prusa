...
bool DesktopIntegrationDialog::is_integrated()
{
    ...
    // confirmation that Anycubic Slicer.desktop exists
    struct stat buffer;   
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        // Copy icon Anycubic Slicer.png from resources_dir()/icons to target_dir_icons/icons/
        if (contains_path_dir(target_candidates[i], "icons")) {
            target_dir_icons = target_candidates[i];
            std::string icon_path = GUI::format("%1%/icons/logo.png",resources_dir());
            std::string dest_path = GUI::format("%1%/icons/%2%logo%3%.png", target_dir_icons, icon_theme_path, version_suffix);
    ...
    if(target_dir_icons.empty()) {
    	...
    	// save path to icon
        app_config->set("desktop_integration_icon_slicer_path", GUI::format("%1%/icons/%2%logo%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        ...
        if (!target_dir_icons.empty())
        {
            std::string icon_path = GUI::format("%1%/icons/logo_192px.png",resources_dir());
            std::string dest_path = GUI::format("%1%/icons/%2%logo%3%.png", target_dir_icons, icon_theme_path, version_suffix);
            if (copy_icon(icon_path, dest_path))
                // save path to icon
...
void DesktopIntegrationDialog::perform_downloader_desktop_integration()
{
    ...
    std::string desktop_file_downloader = GUI::format(
        "[Desktop Entry]\n"
        "Name=AnycubicSlicer URL Protocol%1%\n"
        "Exec=\"%2%\" --single-instance %%u\n"
        "Terminal=false\n"
        "Type=Application\n"
        "MimeType=x-scheme-handler/AnycubicSlicer;\n"
        "StartupNotify=false\n"
        "NoDisplay=true\n"
        , name_suffix, excutable_path);

    // desktop file for downloader as part of main app
    std::string desktop_path = GUI::format("%1%/applications/AnycubicSlicerURLProtocol%2%.desktop", target_dir_desktop, version_suffix);
    if (create_desktop_file(desktop_path, desktop_file_downloader)) {
        ...
        std::string command = GUI::format("xdg-mime default AnycubicSlicerURLProtocol%1%.desktop x-scheme-handler/AnycubicSlicer", version_suffix);
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        if (contains_path_dir(target_candidates[i], "applications")) {
            target_dir_desktop = target_candidates[i];
            // Write slicer desktop file
            std::string path = GUI::format("%1%/applications/AnycubicSlicerURLProtocol%2%.desktop", target_dir_desktop, version_suffix);
