...
#include "slic3r/GUI/Gizmos/GLGizmoLayout.hpp"
...
bool GLGizmosManager::init()
{
    ...
    m_gizmos.emplace_back(new GLGizmoScale3D(m_parent, "icon-scale.svg", EType::Scale,&m_object_manipulation));
    m_gizmos.emplace_back(new GLGizmoFlatten(m_parent, "icon-on face.svg", EType::Flatten));
    m_gizmos.emplace_back(new GLGizmoLayout3D(m_parent, "icon-layout.svg", EType::ModelLayout,&m_object_manipulation));

    m_gizmos.emplace_back(new GLGizmoCut(m_parent, "icon-cut.svg", EType::Cut));
    //m_gizmos.emplace_back(new GLGizmoHollow(m_parent, "ACEmpty.svg", EType::Hollow));
    //m_gizmos.emplace_back(new GLGizmoSlaSupports(m_parent, "ACEmpty.svg", EType::SlaSupports));
    m_gizmos.emplace_back(new GLGizmoFdmSupports(m_parent, "icon-paint on support.svg", EType::FdmSupports));
    m_gizmos.emplace_back(new GLGizmoSeam(m_parent, "icon-seam painting.svg", EType::Seam));
    //m_gizmos.emplace_back(new GLGizmoMmuSegmentation(m_parent, "ACEmpty.svg", EType::MmuSegmentation));
    //m_gizmos.emplace_back(new GLGizmoMeasure(m_parent, "ACEmpty.svg", EType::Measure));
    //m_gizmos.emplace_back(new GLGizmoEmboss(m_parent, "", EType::Emboss));
...
bool GLGizmosManager::init_icon_textures()
{
    ...
    icon_list.clear();
    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-reset-nor.svg", 14, 14, texture_id))
        icon_list.insert(std::make_pair((int)IC_TOOLBAR_RESET, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-reset-hover.svg", 14, 14, texture_id))
        icon_list.insert(std::make_pair((int)IC_TOOLBAR_RESET_HOVER, texture_id));
    else
        return false;

    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/ACEmpty.svg", 30, 22, texture_id))
    ...
    if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/ACEmpty.svg", 30, 22, texture_id))
     ...
     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/ACEmpty.svg", 20, 20, texture_id))
     ...
     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/arrow-put_away-nor.svg", 20, 14, texture_id))
         icon_list.insert(std::make_pair((int) IC_COMBO_UP, texture_id));
     else
         return false;

     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/arrow-expand-nor.svg", 20, 14, texture_id))
         icon_list.insert(std::make_pair((int) IC_COMBO_DOWN, texture_id));
     else
         return false;

     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-custom-nor.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_CUSTOM_NOR, texture_id));
     else
         return false;
     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-custom-sel.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_CUSTOM_SEL, texture_id));
     else
         return false;

     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-fast-nor.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_FAST_NOR, texture_id));
     else
         return false;
     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-fast-sel.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_FAST_SEL, texture_id));
     else
         return false;

     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-H-quality-nor.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_QUALITY_NOR, texture_id));
     else
         return false;
     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-H-quality-sel.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_QUALITY_SEL, texture_id));
     else
         return false;

     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-normal-nor.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_NORMAL_NOR, texture_id));
     else
         return false;
     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-normal-sel.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_NORMAL_SEL, texture_id));
     else
         return false;

     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-list-model-nor.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_LISTMODEL_NOR, texture_id));
     else
         return false;

     if (IMTexture::load_from_svg_file(Slic3r::resources_dir() + "/icons/icon-list-model-sel.svg", 36, 36, texture_id))
         icon_list.insert(std::make_pair((int) IC_LISTMODEL_SEL, texture_id));
     else
         return false;
...
//            m_parent.set_cursor(GLCanvas3D::Cross);
        ...
        //        GLGizmoCut* cut = dynamic_cast<GLGizmoCut3D*>(get_current());
...
bool GLGizmosManager::generate_icons_texture()
{
    ...
    std::vector<std::pair<int, bool>> states; // bool : (true) active background
