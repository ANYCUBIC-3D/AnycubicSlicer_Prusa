...
void GLGizmoMeasure::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    if (ImGui::BeginTable("Commands", 2)) {
        ...
        if (m_mode == EMode::FeatureSelection && m_hover_id != -1) {
            add_strings_row_to_table(*m_imgui, _u8L("Shift"), ImGuiWrapper::COL_BLUE_LIGHT, _u8L("Enable point selection"), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            ++row_count;
        }

        if (m_selected_features.first.feature.has_value()) {
            add_strings_row_to_table(*m_imgui, _u8L("Delete"), ImGuiWrapper::COL_BLUE_LIGHT, _u8L("Restart selection"), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            ++row_count;
        }
        ...
        if (m_selected_features.first.feature.has_value() || m_selected_features.second.feature.has_value()) {
          add_row_to_table(
            [this]() {
                m_imgui->text_colored(ImGuiWrapper::COL_BLUE_LIGHT, _u8L("Esc"));
            },
            [this]() {
