...
void NotificationManager::PopNotification::render(GLCanvas3D& canvas, float initial_y, bool move_from_overlay, float overlay_width)
{
	...
	if (m_state == EState::Hidden) {
        m_top_y = initial_y - GAP_WIDTH * _scaleIndex;
		return;
	}
	...
	float         right_gap = (cnv_size.get_width() - m_window_width)/2;
    ...
    //ImVec2 win_pos(1.0f * (float) cnv_size.get_width() - right_gap, 1.0f * (float) cnv_size.get_height() - 60.0f - m_window_height);
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f * _scaleIndex);
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0.0f,0.0f));
    ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.0f * _scaleIndex);
	...
	if (imgui.begin(name, flags)) {
		...
		render_close_button(imgui, win_size.x, win_size.y, win_pos.x, win_pos.y,true);
...
void NotificationManager::PopNotification::count_lines()
{
	...
	if (text.empty())
		return;
    float m_window_width_gap = m_window_width - m_window_width_offset - 10.0f;
	m_endlines.clear();
	while (last_end < text.length() - 1)
	{
		size_t next_hard_end = text.find_first_of('\n', last_end);
		if (next_hard_end != std::string::npos && ImGui::CalcTextSize(text.substr(last_end, next_hard_end - last_end).c_str()).x < m_window_width_gap) {
            ...
            if (ImGui::CalcTextSize(text.substr(last_end).c_str()).x >= m_window_width_gap) {
				...
				if (next_space > 0 && next_space < text.length()) {
                    ...
                    while (next_space_candidate > 0 &&
                           ImGui::CalcTextSize(text.substr(last_end, next_space_candidate - last_end).c_str()).x < m_window_width_gap) {
                ...
                if (ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x > m_window_width_gap ||
                    ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x < (m_window_width_gap) / 4 * 3
					) {
					float width_of_a = ImGui::CalcTextSize("a").x;
                    int   letter_count = (int) ((m_window_width_gap) / width_of_a);
					while (last_end + letter_count < text.size() && ImGui::CalcTextSize(text.substr(last_end, letter_count).c_str()).x < m_window_width_gap) {
	...
	*/
	...
	if (!m_hypertext.empty()) {
        if (size_of_last_line + ImGui::CalcTextSize(m_hypertext.c_str()).x > m_window_width_gap) {
	...
	if (!m_text2.empty()) {
        ...
        if (size_of_last_line >= m_window_width_gap - ImGui::CalcTextSize(text.substr(0, first_end).c_str()).x) {
		...
		{
			size_t next_hard_end = text.find_first_of('\n', last_end);
			if (next_hard_end != std::string::npos && ImGui::CalcTextSize(text.substr(last_end, next_hard_end - last_end).c_str()).x < m_window_width_gap - size_of_last_line) {
				//next line is ended by '/n'
				...
				// find next suitable endline
                if (ImGui::CalcTextSize(text.substr(last_end).c_str()).x >= m_window_width_gap - size_of_last_line) {
					// more than one line till end
					size_t next_space = text.find_first_of(' ', last_end);
					if (next_space > 0) {
						size_t next_space_candidate = text.find_first_of(' ', next_space + 1);
						while (next_space_candidate > 0 && ImGui::CalcTextSize(text.substr(last_end, next_space_candidate - last_end).c_str()).x < m_window_width_gap - size_of_last_line) {
							next_space = next_space_candidate;
							next_space_candidate = text.find_first_of(' ', next_space + 1);
						}
					}
					...
					// when one word longer than line.
					if (ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x > m_window_width_gap - size_of_last_line ||
						ImGui::CalcTextSize(text.substr(last_end, next_space - last_end).c_str()).x + size_of_last_line < (m_window_width_gap) / 5 * 3
						) {
						float width_of_a = ImGui::CalcTextSize("a").x;
                        int   letter_count = (int) ((m_window_width_gap - size_of_last_line) / width_of_a);
						while (last_end + letter_count < text.size() && ImGui::CalcTextSize(text.substr(last_end, letter_count).c_str()).x < m_window_width_gap - size_of_last_line) {
							letter_count += get_utf8_sequence_length(text, last_end + letter_count);
						}
	...
	for (size_t i = 0; i < (m_multiline ? m_endlines.size() : std::min(m_endlines.size(), m_normal_lines_count)); i++) {
		...
		if (m_endlines.size() > i && m_text1.size() >= m_endlines[i]) {
			if (i == m_normal_lines_count - 1 && m_endlines.size() > m_normal_lines_count && !m_multiline) {
				...
				while (ImGui::CalcTextSize(line.c_str()).x > m_window_width_gap - ImGui::CalcTextSize(("  [" + _u8L("More") + "]").c_str()).x) {
    ...
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
        ...
        if (m_endlines.size() > i && m_text1.size() >= m_endlines[i]) {
            if (i == m_normal_lines_count - 1 && m_endlines.size() > m_normal_lines_count && !m_multiline) {
                ...
                while (ImGui::CalcTextSize(line.c_str()).x >
                       m_window_width_gap - ImGui::CalcTextSize(("  [" + _u8L("More") + "]").c_str()).x) {
                    line = line.substr(0, line.length() - 1);
                }
                line += " "; //"..";
            } else {
    ...
    // hyperlink text
    if (!m_multiline && m_lines_count > m_normal_lines_count) {
        /*render_hypertext(imgui, x_offset + ImGui::CalcTextSize((line + " ").c_str()).x, starting_y + (m_normal_lines_count - 1) * shift_y,
                         "[" + _u8L("More") + "]", true);*/
        render_hypertext(_new, imgui, x_offset + ImGui::CalcTextSize((line + " ").c_str()).x,
                         starting_y + (m_normal_lines_count - 1) * shift_y,
                         "[" + _u8L("More") + "]", true);
    } else if (!m_hypertext.empty()) {
        ...
        render_hypertext(_new, imgui, x_offset + ImGui::CalcTextSize((line + (line.empty() ? "" : " ")).c_str()).x,
                         starting_y + (m_endlines.size() - 1) * shift_y, m_hypertext);
    }
...
void NotificationManager::PopNotification::render_hypertext(
    bool _new,ImGuiWrapper &imgui, const float text_x, const float text_y, const std::string text, bool more)
{
    /*// invisible button
    ImVec2 part_size = ImGui::CalcTextSize(text.c_str());
    ...
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImGuiWrapper::COL_WINDOW_BG);
    ImGui::PushStyleColor(ImGuiCol_Button, ImGuiWrapper::COL_WINDOW_BG);
    ImGui::PushStyleColor(ImGuiCol_Border, ImGuiWrapper::COL_WINDOW_BG);
    ImGui::PushStyleColor(ImGuiCol_Text, ImGuiWrapper::COL_AC_BLUE);

    ImGui::SetCursorPosX(text_x - 4.0f);
    ImGui::SetCursorPosY(text_y);
    ...
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
	...
	float h_gap = 4.0f;
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(h_gap, h_gap));
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, 6.0f);
    ...
    ImGui::PushStyleColor(ImGuiCol_Text, ImGuiWrapper::COL_AC_BLUE);
    wxString _btn_str   = _u8L("OK");
    ImVec2   text_size = ImGui::CalcTextSize(_btn_str.c_str());
    ImGui::SetCursorPosX(win_size.x * 0.90);
    ImGui::SetCursorPosY((win_size_y - text_size.y) / 2 - 8.0f);
    if (ImGui::Button(_btn_str.c_str())) {
        close();
    }
    ImGui::PopStyleColor(4);
    ImGui::PopStyleVar(2);
...
void NotificationManager::PopNotification::render_minimize_button(ImGuiWrapper &imgui, const float win_pos_x, const float win_pos_y,const bool _new)
{
    //ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.0f, .0f, .0f, .0f));
    //ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(.0f, .0f, .0f, .0f));
...
{
    if (m_eject_pending) {
        return PopNotification::render_text(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y, _new);
    }
    float       x_offset = m_left_indentation;
    std::string fulltext = m_text1 + m_hypertext; //+ m_text2;
    // Lines are always at least two and m_multiline is always true for ExportFinishedNotification.
    ...
    for (size_t i = 0; i < m_lines_count; i++) {
        ...
        if (m_text1.size() >= m_endlines[i]) {
            ...
            // hyperlink text
            if (i == 0 && !m_eject_pending) {
                render_hypertext(_new, imgui, x_offset + ImGui::CalcTextSize(line.c_str()).x + ImGui::CalcTextSize("   ").x, starting_y,
                                 _u8L("Open Folder."));
            }
        }
    }
...
void NotificationManager::ExportFinishedNotification::render_close_button(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y, const bool _new)
{
    PopNotification::render_close_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y, _new);
    if (m_to_removable && !m_eject_pending)
        render_eject_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
}
    ...
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
    ...
    if (m_multiline) {
        ...
        if (m_endlines[0] > m_text1.size())
            return;
        // one line text, one line bar
        ImVec2 mtext_size = ImGui::CalcTextSize(m_text1.c_str());
        ImGui::SetCursorPosX(m_left_indentation);
        ImGui::SetCursorPosY((win_size_y - mtext_size.y) / 2);
        imgui.text(m_text1.substr(0, m_endlines[0]).c_str());
        if (m_has_cancel_button)
            render_cancel_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y,true);
        render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y,true);
    }
}
    ...
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
    ...
    if (m_render_percentage) {
        ...
        ImGui::SetCursorPosX(win_size_x * 0.7);
        ImGui::SetCursorPosY((win_size_y - text_size.y) / 2 );
...
void NotificationManager::ProgressBarWithCancelNotification::render_close_button(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
    if (m_percentage < 0.f || m_percentage >= 1.f)
        render_close_button_inner(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
...
void NotificationManager::ProgressBarWithCancelNotification::render_bar(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
    ProgressBarNotification::render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
    std::string text;
...
void NotificationManager::URLDownloadNotification::render_bar(ImGuiWrapper& imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
	auto shorten_to_line = [this](const std::string& text, bool dots) -> std::string {
		...
		while (ImGui::CalcTextSize(line.c_str()).x > m_window_width - m_window_width_offset - 10.0f) {
			line = line.substr(0, line.length() - 1);
			did_shorten = true;
		}
		if (did_shorten && dots) {
...
void NotificationManager::PrintHostUploadNotification::generate_text()
{
	...
	std::string line1;
	if (ImGui::CalcTextSize(m_filename.c_str()).x > m_window_width - m_window_width_offset - 10.0f) {
	...
	if (ImGui::CalcTextSize((line1 + line2).c_str()).x > m_window_width - m_window_width_offset - 10.0f)
...
void NotificationManager::PrintHostUploadNotification::render_text(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
    // If not completed, the text rendering is very similar to progressbar notification except it doesnt use m_multiline to decide.
    // If completed, whole text is part of m_text_1 and is rendered by PopNotification function.
...
void NotificationManager::PrintHostUploadNotification::render_close_button(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
    if (m_has_cancel_button)
        render_cancel_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
...
void NotificationManager::SlicingProgressNotification::render_text(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
    if (m_sp_state == SlicingProgressState::SP_PROGRESS /*|| (m_sp_state == SlicingProgressState::SP_COMPLETED && !m_sidebar_collapsed)*/) {
        //ProgressBarNotification::render_text(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
        ProgressBarNotification::render_text(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y,true);
        /* // enable for hypertext during slicing (correct call of export_enabled needed)
        if (m_multiline) {
            // two lines text, one line bar
            ImGui::SetCursorPosX(m_left_indentation);
        ...
        */
        ...
        if (m_sidebar_collapsed && m_has_print_info) {
            x_offset = 20;
            cursor_y = win_size.y / 2 + win_size.y / 4.5 - text_size.y / 2;
            ImGui::SetCursorPosX(x_offset);
            ImGui::SetCursorPosY(cursor_y);
            imgui.text(m_print_info.c_str());
            cursor_y = win_size.y / 2 - win_size.y / 6 - text_size.y / 2;
        ...
        imgui.text(m_text1.c_str());
        if (m_sidebar_collapsed)
            render_hypertext(true,imgui, x_offset + text1_size.x + 4, cursor_y, m_hypertext);
    } else {
        PopNotification::render_text(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y);
    }
}
...
void NotificationManager::SlicingProgressNotification::render_bar(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
    if (m_sp_state != SlicingProgressState::SP_PROGRESS) {
        return;
    }
    ProgressBarNotification::render_bar(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y,true);
}
...
void NotificationManager::SlicingProgressNotification::render_hypertext(
    bool _new,ImGuiWrapper &imgui, const float text_x, const float text_y, const std::string text, bool more)
{
    if (m_sp_state == SlicingProgressState::SP_COMPLETED && !m_sidebar_collapsed)
        return;
    ProgressBarNotification::render_hypertext(_new,imgui, text_x, text_y, text, more);
}
...
void NotificationManager::SlicingProgressNotification::render_cancel_button(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,bool const _new)
{
    ImVec2 win_size(win_size_x, win_size_y);
    ImVec2 win_pos(win_pos_x, win_pos_y);
    /*ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(.0f, .0f, .0f, .0f));
    ...
    ImGui::PushStyleColor(ImGuiCol_Border, ImGuiWrapper::COL_AC_BLUE);
    ImGui::PushStyleColor(ImGuiCol_Text, ImGuiWrapper::COL_AC_BLUE);
    wxString btn_str   = _u8L("Cancel");
    ImVec2            text_size = ImGui::CalcTextSize(btn_str.c_str());
    ImGui::SetCursorPosX(win_size.x * 0.8);
...
void NotificationManager::SlicingProgressNotification::render_close_button(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y,const bool _new)
{
    // Do not render close button while showing progress - cancel button is rendered instead
    if (m_sp_state != SlicingProgressState::SP_PROGRESS) {
        ProgressBarNotification::render_close_button(imgui, win_size_x, win_size_y, win_pos_x, win_pos_y, _new);
    }
}
...
void NotificationManager::render_notifications(GLCanvas3D& canvas, float overlay_width)
{
    ...
    ImGuiWrapper &imgui       = *wxGetApp().imgui();
    float         _scaleIndex = imgui.get_style_scaling();
    float         last_y      = GAP_WIDTH * _scaleIndex + (m_in_preview ? 70.0f * _scaleIndex : 0);

	for (const auto& notification : m_pop_notifications) {
		if (notification->get_state() != PopNotification::EState::Hidden) {
			notification->render(canvas, last_y, m_move_from_overlay && !m_in_preview, overlay_width);
			if (notification->get_state() != PopNotification::EState::Finished)
                last_y = notification->get_top() + GAP_WIDTH * _scaleIndex;
		}
