...
protected:
    ...
    ACButton *add_button(wxWindowID btn_id, bool set_focus = false, const wxString &label = wxString());
	// returns pointer to found button or NULL
	//wxButton* get_button(wxWindowID btn_id);
    ACButton *get_button(wxWindowID btn_id);
    ...
    ACButton *      yes_button;
    ACButton *      no_button;
    ACButton *      cancel_button;

public:
    void SetOkBtnLabel(const wxString &label)
    {
        if (ok_button)
            ok_button->SetLabel(label);
    }
    void SetYesBtnLabel(const wxString &label)
    {
        if (yes_button)
            yes_button->SetLabel(label);
    }
    void SetNoBtnLabel(const wxString &label)
    {
        if (no_button)
            no_button->SetLabel(label);
    }
    void SetCancelBtnLabel(const wxString &label)
    {
        if (cancel_button)
            cancel_button->SetLabel(label);
    }
};
