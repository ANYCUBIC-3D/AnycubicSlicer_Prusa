...
//#include "libslic3r/ToolOrdering.hpp"
...
std::set<int> TickCodeInfo::get_used_extruders_for_tick(int tick, int only_extruder, double print_z, Mode force_mode /* = Undef*/) const
{
    ...
    if (e_mode == MultiExtruder) {
        ...
        auto it_layer_tools = std::lower_bound(tool_ordering.begin(), tool_ordering.end(), tool_ordering.tools_for_layer(print_z));
...
bool ACIMSlider::render(int canvas_width, int canvas_height)
{
    ...
    ImGui::PushStyleColor(ImGuiCol_::ImGuiCol_Text, ImGuiWrapper::COL_AC_WHITE);
    ...
    if (is_horizontal()) {
        ...
        float  pos_x = 0.2f * canvas_width;
        float  pos_y = (canvas_height - HORIZONTAL_SLIDER_SIZE.y * m_scale);
        float  canvas_right = 320 * m_scale;
        float  _slider_w    = canvas_width - pos_x - canvas_right;
        ImVec2 size         = ImVec2(_slider_w, HORIZONTAL_SLIDER_SIZE.y * m_scale);
        ...
        float spacingH = 10*m_scale;
        float paddingH = 10*m_scale;
        float sliderH = canvas_height - buttonsH - paddingH*2 - onelayerBtH - spacingH;
        float pos_y = paddingH;
        ImVec2 size = ImVec2((VERTICAL_SLIDER_SIZE.x + TEXT_WIDTH_DUMMY * scale - TEXT_WIDTH_DUMMY) * m_scale, sliderH);
        imgui.set_next_window_pos(pos_x, pos_y, ImGuiCond_Always);
