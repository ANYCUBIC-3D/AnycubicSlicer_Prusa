...
void GCodeViewer::load_toolpaths(const GCodeProcessorResult& gcode_result)
{
    ...
    wxProgressDialog *progress_dialog = wxGetApp().is_gcode_viewer() || wxGetApp().plater()->acgcode_mode() == ACGcodeMode::ACGcode_Show ?
...
void GCodeViewer::render_legend(float& legend_height)
{
    ...
    auto used_filament_per_role = [this, imperial_units](GCodeExtrusionRole role) {
        ...
        double koef = imperial_units ? 1000.0 / in_to_mm : 1.0;
    ...
    auto get_used_filament_from_volume = [this, imperial_units](double volume, int extruder_id) {
        double koef = imperial_units ? 1.0 / in_to_mm : 0.001;
    ...
    imgui.combo(wxString("##GCodeViewType"), { _u8L("Feature type"),
                                     _u8L("Height (mm)"),
                                     _u8L("Width (mm)"),
                                     _u8L("Speed (mm/s)"),
                                     _u8L("Fan speed (%)"),
    ...
    bool show_settings = wxGetApp().is_gcode_viewer() || wxGetApp().plater()->acgcode_mode() == ACGcodeMode::ACGcode_Show;
    show_settings &= (m_view_type == EViewType::FeatureType || m_view_type == EViewType::Tool);
    show_settings &= has_settings;
    if (show_settings || wxGetApp().plater()->acgcode_mode() == ACGcodeMode::ACGcode_Show) {
        ImGui::Spacing();
    ...
    if (show_estimated_time) {
        ...
        if (ImGui::BeginTable("Times", 2)) {
            ...
            bool imperial_units = wxGetApp().app_config->get_bool("use_inches");
            double koef = imperial_units ? in_to_mm : 1000.0;
            ...
            koef = imperial_units ? pow(mm_to_in, 3) : 1.0f;
            ...
            if (!time_mode.layers_times.empty()) {
                add_strings_row_to_table(_u8L("First layer") + ":", ImGuiWrapper::COL_AC_BLACK,
                    short_time_ui(get_time_dhms(time_mode.layers_times.front())), ImGuiWrapper::COL_AC_BLACK);
            }

            add_strings_row_to_table(_u8L("Total") + ":", ImGuiWrapper::COL_AC_BLACK,
                short_time_ui(get_time_dhms(time_mode.time)), ImGuiWrapper::COL_AC_BLACK);
        ...
        std::function<void(ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel)> draw_callback)
    {
        ...
        if (imgui.draw_radio_button(name, 2.3f * icon_size, false, [draw_callback, active](ImGuiWindow& window, const ImVec2& pos, float size){
    ...
    ImGui::Spacing();
    ImGui::Spacing();
    toggle_button(Preview::OptionType::Travel, _u8L("Travel"), [&imgui](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
    ...
    ImGui::SameLine();
    toggle_button(Preview::OptionType::Retractions, _u8L("Retractions"), [&imgui](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
        const wchar_t& icon_id = isSel ? ImGui::LegendSelRetract : isHov ? ImGui::LegendHovRetract : ImGui::LegendNorRetract;
        imgui.draw_icon(window, pos, size, icon_id);
        });
    ImGui::SameLine();
    toggle_button(Preview::OptionType::Unretractions, _u8L("Deretractions"), [&imgui](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
        const wchar_t& icon_id = isSel ? ImGui::LegendSelDeretract : isHov ? ImGui::LegendHovDeretract : ImGui::LegendNorDeretract;
        imgui.draw_icon(window, pos, size, icon_id);
        });
    ImGui::SameLine();
    toggle_button(Preview::OptionType::Seams, _u8L("Seams"), [&imgui](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
   ...
   // toggle_button(Preview::OptionType::CustomGCodes, _u8L("Custom G-codes"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size) {
    ...
    if (!wxGetApp().is_gcode_viewer() || !(wxGetApp().plater()->acgcode_mode() == ACGcodeMode::ACGcode_Show)) {
        ImGui::SameLine();
        toggle_button(Preview::OptionType::Shells, _u8L("Shells"), [&imgui](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
