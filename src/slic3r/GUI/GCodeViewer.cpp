...
void GCodeViewer::COG::render()
{
    ...
    //imgui.text_colored(ImGuiWrapper::COL_BLUE_LIGHT, _u8L("Center of mass") + ":");
    //ImGui::SameLine();
    //char buf[1024];
...
void GCodeViewer::SequentialView::GCodeWindow::render(float top, float bottom, uint64_t curr_line_id) const
{
    ...
    static const ImVec4 LINE_NUMBER_COLOR    = ImGuiWrapper::COL_GREY_DARK;
    static const ImVec4 SELECTION_RECT_COLOR = ImGuiWrapper::COL_BLUE_DARK;
    static const ImVec4 COMMAND_COLOR        = ImGuiWrapper::COL_BLUE_DARK;
    static const ImVec4 PARAMETERS_COLOR     = { 0.3f, 0.3f, 0.3f, 1.0f };
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImGuiWrapper::COL_BLUE_LIGHT);
...
void GCodeViewer::export_toolpaths_to_obj(const char* filename) const
{
    ...
    fprintf(fp, "# Generated by %s-%s based on PrusaSlicer\n", SLIC3R_APP_NAME, SLIC3R_VERSION);
...
void GCodeViewer::render_legend(float& legend_height)
{
    ...
    imgui.begin(_u8L("Legend"), ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
        ...
        if (callback != nullptr) {
            ...
            if (!time.empty()) {
                ...
                /*draw_list->AddRectFilled({ pos.x, pos.y + 2.0f }, { pos.x + width, pos.y + icon_size - 2.0f },
                    ImGui::GetColorU32(ImGuiWrapper::COL_AC_WHITE));*/
                draw_list->AddRectFilled({pos.x, pos.y + 2.0f}, {pos.x + width, pos.y + icon_size - 2.0f},
                                         ImGui::GetColorU32(ImGuiWrapper::COL_AC_BLUE));
    ...
    ImGui::PushStyleColor(ImGuiCol_FrameBg, ImGuiWrapper::COL_AC_PANELGRAY );
    ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImGuiWrapper::COL_AC_LIGHTBLUE);
    ImGui::PushStyleColor(ImGuiCol_PopupBg, ImGuiWrapper::COL_AC_PANELGRAY);
    imgui.combo("##GCodeViweType", { _u8L("Feature type"),
                                     _u8L("Height (mm)"),
                                     _u8L("Width (mm)"),
                                     _u8L("Speed (mm/s)"),
                                     _u8L("Fan speed (%)"),
    ...
    if (show_settings) {
        ...
        if (ImGui::BeginTable("Settings", 2)) {
            if (!m_settings_ids.printer.empty())
                add_strings_row_to_table(_u8L("Printer") + ":", ImGuiWrapper::COL_GREY_DARK,
                    trim_text_if_needed(m_settings_ids.printer), ImGuiWrapper::to_ImVec4(ColorRGBA::BLACK()));
            if (!m_settings_ids.print.empty())
                add_strings_row_to_table(_u8L("Print settings") + ":", ImGuiWrapper::COL_GREY_DARK,
                    trim_text_if_needed(m_settings_ids.print), ImGuiWrapper::to_ImVec4(ColorRGBA::BLACK()));
            if (!m_settings_ids.filament.empty()) {
                for (unsigned char i : m_extruder_ids) {
                    if (i < static_cast<unsigned char>(m_settings_ids.filament.size()) && !m_settings_ids.filament[i].empty()) {
                        ...
                        add_strings_row_to_table(txt, ImGuiWrapper::COL_GREY_DARK,
                            trim_text_if_needed(m_settings_ids.filament[i]), ImGuiWrapper::to_ImVec4(ColorRGBA::BLACK()));
                    }
                }
            }
            ImGui::EndTable();
    ...
    if (show_estimated_time) {
        ...
        if (ImGui::BeginTable("Times", 2)) {
            ...
            wxString new_label = _L("Used filament");
            //if (is_wipe_tower)
            ...
            wxString info_text = /*is_wipe_tower ?
                                ...
                                wxString::Format("%.2f%s", ps.total_used_filament / koef, (imperial_units ? "in" : "m"));


            add_strings_row_to_table(into_u8(new_label)+":", ImGuiWrapper::COL_AC_BLACK, into_u8(info_text), ImGuiWrapper::COL_AC_BLACK);

            koef = imperial_units ? pow(ObjectManipulation::mm_to_in, 3) : 1.0f;

            //info_text = wxString::Format("%.2f%s", (imperial_units ? ps.total_extruded_volume * koef : ps.total_extruded_volume), into_u8(imperial_units ? _L("in³") : _L("mm³")));
            //add_strings_row_to_table(into_u8(new_label)+":", ImGuiWrapper::COL_AC_BLACK, into_u8(info_text), ImGuiWrapper::COL_AC_BLACK);

            info_text = wxString::Format("%.2fg", ps.total_weight);
            add_strings_row_to_table(into_u8(new_label)+":", ImGuiWrapper::COL_AC_BLACK, into_u8(info_text), ImGuiWrapper::COL_AC_BLACK);



            if (!time_mode.layers_times.empty()) {
                /*add_strings_row_to_table(_u8L("First layer") + ":", ImGuiWrapper::COL_BLUE_LIGHT,
                    short_time(get_time_dhms(time_mode.layers_times.front())), ImGuiWrapper::to_ImVec4(ColorRGBA::WHITE()));*/
                add_strings_row_to_table(_u8L("First layer") + ":", ImGuiWrapper::COL_AC_BLACK,
                                         short_time(get_time_dhms(time_mode.layers_times.front())), ImGuiWrapper::COL_AC_BLACK);
            }

            /*add_strings_row_to_table(_u8L("Total") + ":", ImGuiWrapper::COL_BLUE_LIGHT,
                short_time(get_time_dhms(time_mode.time)), ImGuiWrapper::to_ImVec4(ColorRGBA::WHITE()));*/
            add_strings_row_to_table(_u8L("Total") + ":", ImGuiWrapper::COL_AC_BLACK, short_time(get_time_dhms(time_mode.time)),
                                     ImGuiWrapper::COL_AC_BLACK);
    ...
    // toolbar section
    auto toggle_button = [this, &imgui, icon_size](Preview::OptionType type, 
        const std::string& name,
        std::function<void(ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel)> draw_callback) 
    {
        ...
        if (imgui.draw_radio_button(name, 1.5f * icon_size, false, [draw_callback, active](ImGuiWindow& window, const ImVec2& pos, float size){
                draw_callback(window, pos, size, ImGui::IsItemHovered(), active);
            }, false)) {
    ...
    ImGui::Spacing();
    ImGui::Spacing();
    toggle_button(Preview::OptionType::Travel, _u8L("Travel"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
        const wchar_t& icon_id = isSel ? ImGui::LegendSelTravel : isHov ? ImGui::LegendHovTravel : ImGui::LegendNorTravel;
        image_icon(window, pos, size, icon_id);
        });
    //ImGui::SameLine();
    //toggle_button(Preview::OptionType::Wipe, _u8L("Wipe"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
    //    image_icon(window, pos, size, ImGui::LegendWipe);
    //    });
    ImGui::SameLine();
    toggle_button(Preview::OptionType::Retractions, _u8L("Retractions"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
        const wchar_t& icon_id = isSel ? ImGui::LegendSelRetract : isHov ? ImGui::LegendHovRetract : ImGui::LegendNorRetract;
        image_icon(window, pos, size, icon_id);
        });
    ImGui::SameLine();
    toggle_button(Preview::OptionType::Unretractions, _u8L("Deretractions"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
        const wchar_t& icon_id = isSel ? ImGui::LegendSelDeretract : isHov ? ImGui::LegendHovDeretract : ImGui::LegendNorDeretract;
        image_icon(window, pos, size, icon_id);
        });
    ImGui::SameLine();
    toggle_button(Preview::OptionType::Seams, _u8L("Seams"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
        const wchar_t& icon_id = isSel ? ImGui::LegendSelSeams : isHov ? ImGui::LegendHovSeams : ImGui::LegendNorSeams;
        image_icon(window, pos, size, icon_id);
        });
    //ImGui::SameLine();
    //toggle_button(Preview::OptionType::ToolChanges, _u8L("Tool changes"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
    //    image_icon(window, pos, size, ImGui::LegendToolChanges);
    //    });
    //ImGui::SameLine();
    //toggle_button(Preview::OptionType::ColorChanges, _u8L("Color changes"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
    //    image_icon(window, pos, size, ImGui::LegendColorChanges);
    //    });
    //ImGui::SameLine();
    //toggle_button(Preview::OptionType::PausePrints, _u8L("Print pauses"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
    //    image_icon(window, pos, size, ImGui::LegendPausePrints);
    //    });
    //ImGui::SameLine();
    //toggle_button(Preview::OptionType::CustomGCodes, _u8L("Custom G-codes"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
    //    image_icon(window, pos, size, ImGui::LegendCustomGCodes);
    //    });
    //ImGui::SameLine();
    //toggle_button(Preview::OptionType::CenterOfGravity, _u8L("Center of gravity"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
    //    image_icon(window, pos, size, ImGui::LegendCOG);
    //    });
    if (!wxGetApp().is_gcode_viewer()) {
        ImGui::SameLine();
        toggle_button(Preview::OptionType::Shells, _u8L("Shells"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
            const wchar_t& icon_id = isSel ? ImGui::LegendSelShells : isHov ? ImGui::LegendHovShells : ImGui::LegendNorShells;
            image_icon(window, pos, size, icon_id);
            });
        //ImGui::SameLine();
    }
    //toggle_button(Preview::OptionType::ToolMarker, _u8L("Tool marker"), [image_icon](ImGuiWindow& window, const ImVec2& pos, float size, bool isHov, bool isSel) {
...
void GCodeViewer::render_slider(int canvas_width, int canvas_height) {
    ...
    m_moves_slider->set_scale(_scaleIndex);
    m_layers_slider->set_scale(_scaleIndex);
