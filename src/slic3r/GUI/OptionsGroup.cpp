    ...
    for (const Option& opt : option_set) {
        ...
        if (!custom_ctrl) {
            ...
            if (!option.sidetext.empty() || sidetext_width > 0) {
                ...
                h_sizer->Add(sidetext, 0, wxLEFT | wxALIGN_CENTER_VERTICAL, 14);
            }
...
bool OptionsGroup::activate(std::function<void()> throw_if_canceled/* = [](){}*/, int horiz_alignment/* = wxALIGN_LEFT*/)
{
	...
	try {
		if (staticbox) {
            ...
            stb->SetFont(stb->GetFont().MakeBold());
		}
		else
			stb = nullptr;
...
void ConfigOptionsGroup::sys_color_changed()
{
    ...
    if (staticbox && stb) {
        ...
        // update bitmaps for extra column items (like "ACEmpty" buttons on settings panel)
        for (auto extra_col : m_extra_column_item_ptrs)
...
wxString OptionsGroup::get_url(const std::string& path_end)
{
    ...
    return wxString("https://help.xxxxxxx.com/") + lang_marker + wxString("/article/" + path_end);
}
