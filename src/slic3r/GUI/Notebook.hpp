...
public:
    ...
    bool     InsertPage(size_t             n,
                        ...
                        const std::string &bmp_name_hover    = "",
...
private:
    ...
    std::vector<std::string>        m_pageButtons_img;
    std::vector<std::string>        m_pageButtons_img_click;
    int                             m_selection {-1};
    int                             m_btn_margin;
    int                             m_line_margin;
};
...
public:
    ...
    bool AddPage(wxWindow *page, const wxString &text, const std::string &bmp_name, const std::string &bmp_click_name, bool bSelect = false)
    {
        DoInvalidateBestSize();
        return InsertPage(GetPageCount(), page, text, bmp_name, bmp_click_name, bSelect);
    }
    ...
    bool InsertPage(size_t             n,
                    ...
                    const wxString &   text,
                    const std::string &bmp_name,
                    const std::string &bmp_click_name,
                    bool               bSelect  = false)
    {
        if (!wxBookCtrlBase::InsertPage(n, page, text, bSelect))
            return false;

        GetBtnsListCtrl()->InsertPage(n, text, bSelect, bmp_name, bmp_click_name);
