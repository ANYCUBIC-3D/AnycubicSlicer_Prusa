...
constexpr auto PRESRT_BTN_HEIGHT = 25;
constexpr auto PRESRT_BTN_WIDTH  = 75;
...
SavePresetDialog::Item::Item(Preset::Type type, const std::string& suffix, wxBoxSizer* sizer, SavePresetDialog* parent, bool is_for_multiple_save):
    ...
    m_parent(parent),
    m_valid_bmp(new wxStaticBitmap(m_parent, wxID_ANY, *get_bmp_bundle("ico-tick_mark"))),
...
{
    std::string bmp_name =  m_valid_type == ValidationType::Warning ? "ico-exclamation" :
                            m_valid_type == ValidationType::NoValid ? "icon-cross"       : "ico-tick_mark" ;
...
SavePresetDialog::SavePresetDialog(wxWindow* parent, Preset::Type type, std::string suffix, bool template_filament)
    : DPIDialog(parent,
                ...
                _L("Save preset"),
                wxDefaultPosition,
                wxSize(45 * wxGetApp().em_unit(), 6 * wxGetApp().em_unit()),
                wxNO_BORDER)
...
//    this->SetFont(wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT));
    ...
    ACDialogTopbar* m_dialog_presets = new ACDialogTopbar(this, _L("Save preset"), 45);
    ...
    ok_btn->SetButtonType(ACButton::AC_BUTTON_LV0);


    ok_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent &) { accept(); });
    ok_btn->Bind(wxEVT_UPDATE_UI, [this](wxUpdateUIEvent &evt) { evt.Enable(enable_ok_btn()); });

    ACButton *cancel_btn = new ACButton(this, _L("Cancel"), "", "", "", wxNO_BORDER, wxSize(0, 0));
    cancel_btn->SetMinSize(wxSize(PRESRT_BTN_WIDTH, PRESRT_BTN_HEIGHT));
    cancel_btn->SetPaddingSize(wxSize(10, 0));
    cancel_btn->SetCornerRadius(4);
    cancel_btn->SetButtonType(ACButton::AC_BUTTON_LV1);
    cancel_btn->Bind(wxEVT_BUTTON, &SavePresetDialog::OnCloseFrame, this);
