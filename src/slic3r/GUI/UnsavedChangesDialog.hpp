...
class UnsavedChangesDialog : public DPIDialog
{
    ...
    ACButton *              m_save_btn{nullptr};
    ACButton *              m_transfer_btn{nullptr};
    ACButton *              m_discard_btn{nullptr};
    ...
    //ACDialogTopbar *m_dialogChanges;
...
public:
    ...
    UnsavedChangesDialog(Preset::Type type, PresetCollection* dependent_presets, const std::string& new_selected_preset = std::string(), wxWindow* parent = NULL);
    // show unsaved changes for all another cases
    UnsavedChangesDialog(const wxString& caption, const wxString& header, const std::string& app_config_key, int act_buttons, wxWindow* parent = NULL);
