...
#include "slic3r/GUI/ACConfigWizard.hpp"
...
void PresetUpdater::priv::get_missing_resource(const std::string& vendor, const std::string& filename, const std::string& url) const
{
	...
	//if (!boost::starts_with(url, "http://files.xxxxxxx.com/wp-content/uploads/repository/") &&
	//	!boost::starts_with(url, "https://files.xxxxxxx.com/wp-content/uploads/repository/"))
	//{
	//	throw Slic3r::CriticalException(GUI::format("URL outside xxxxxxx.com network: %1%", url));
	//}
...
void PresetUpdater::priv::sync_config(const VendorMap vendors, const std::string& index_archive_url)
{
	...
	//if (!boost::starts_with(index_archive_url, "http://files.xxxxxxx.com/wp-content/uploads/repository/") &&
	//	!boost::starts_with(index_archive_url, "https://files.xxxxxxx.com/wp-content/uploads/repository/"))
	//{
...
// Version of slic3r that was running the last time and which was read out from Anycubic Slicer.ini is provided
// as a parameter.
Updates PresetUpdater::priv::get_config_updates(const Semver &old_slic3r_version) const
{
    ...
    for (const Index& idx : index_db) {
		...
		if (recommended->config_version < vp.config_version) {
			BOOST_LOG_TRIVIAL(warning) << format("Recommended config version for the currently running AnycubicSlicer is older than the currently installed config for vendor %1%. This should not happen.", idx.vendor());
			continue;
		}
		...
		// Config bundle update situation. The recommended config bundle version for this Anycubic Slicer version from the index from the cache is newer
		...
		// 1) The last config index downloaded by any previously running Anycubic Slicer instance
		// 2) The last config index installed by any previously running Anycubic Slicer instance (older or newer) from its resources.
		// 3) The last config index installed by the currently running Anycubic Slicer instance from its resources.
			...
			if (rsrc_vp.valid()) {
				...
				if (recommended != rsrc_idx.end() && recommended->config_version == rsrc_vp.config_version && recommended->config_version > vp.config_version) {
					...
					BOOST_LOG_TRIVIAL(warning) << format("The recommended config version for vendor `%1%` in resources does not match the recommended\n"
			                                             " config version for this version of AnycubicSlicer. Corrupted installation?", idx.vendor());
				}
			}
		}
	...
	if (updates.incompats.size() > 0) {
		...
		if (res == wxID_REPLACE) {
			...
			if (! p->perform_updates(std::move(updates)) ||
				! GUI::wxGetApp().run_wizard(GUI::ACConfigWizard::RR_DATA_INCOMPAT))
