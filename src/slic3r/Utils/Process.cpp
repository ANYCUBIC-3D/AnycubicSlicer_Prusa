...
// For starting another Anycubic Slicer instance on OSX.
...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance, bool delete_after_load)
{
	...
	path += "\\";
	path += (instance_type == NewSlicerInstanceType::Slicer) ? "Anycubic-Slicer.exe" : "AnycubicSlicer-gcodeviewer.exe";
	...
	// Don't call with wxEXEC_HIDE_CONSOLE, Anycubic Slicer in GUI mode would just show the splash screen. It would not open the main window though, it would
...
#if defined(__APPLE__)
	{
		...
		// ((instance_type == NewSlicerInstanceType::Slicer) ? "AnycubicSlicer" : "PrusaGCodeViewer");
		// Just run Anycubic Slicer and give it a --gcodeviewer parameter.
...
#else // Linux or Unix
	{
		...
		if (args.empty()) {
			...
			my_path = (bin_path.parent_path() / ((instance_type == NewSlicerInstanceType::Slicer) ? "Anycubic-Slicer" : "AnycubicSlicer-gcodeviewer")).string();
