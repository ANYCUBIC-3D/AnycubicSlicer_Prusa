...
int CLI::run(int argc, char **argv)
{
    ...
    if (!start_as_gcodeviewer) {
        for (const std::string& file : m_input_files) {
            if (boost::starts_with(file, "anycubicslicer://")) {
...
bool CLI::setup(int argc, char **argv)
{
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into Anycubic Slicer process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into a Anycubic Slicer process, it happens at the very start of the application,
	...
	set_profiles_anycubic_dir((path_resources / "profiles" / "Anycubic").string());
...
#endif /* SLIC3R_GUI */
        << std::endl
        << "https://github.com/ANYCUBIC-3D/AnycubicSlicer" << std::endl << std::endl
...
extern "C" {
    // Enable printing stacktrace by default. It can be disabled by running Anycubic Slicer with "UBSAN_OPTIONS=print_stacktrace=0".
