...
bool ImGui::ACBeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, float arrowWidth)
{
    // Always consume the SetNextWindowSizeConstraint() call in our early return paths
    ImGuiContext& g = *GImGui;
    bool has_window_size_constraint = (g.NextWindowData.Flags & ImGuiNextWindowDataFlags_HasSizeConstraint) != 0;
    ...
    const float arrow_size = (flags & ImGuiComboFlags_NoArrowButton) ? arrowWidth : GetFrameHeight();
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float expected_w = CalcItemWidth();
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : expected_w;
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
...
bool ImGui::Selectable(const int &rounding,const char *label, bool selected, ImGuiSelectableFlags flags, const ImVec2 &size_arg)
{
    ImGuiWindow *window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (hovered || selected) {
        const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
        int x_gap = 2;
        RenderFrame(x_gap, bb.Min, bb.Max, col, true, rounding);
        RenderNavHighlight(bb, id, ImGuiNavHighlightFlags_TypeThin | ImGuiNavHighlightFlags_NoRounding);
    }
...
bool ImGui::ListBox(const int & rounding,
                    const char *label,
                    int *       current_item,
                    ...
                    int   height_in_items)
{
    ...
    while (clipper.Step())
        for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++) {
            ...
            PushID(i);
            const bool item_selected = (i == *current_item);
            if (Selectable(rounding,item_text, item_selected)) {
                *current_item = i;
                value_changed = true;
            }
            if (item_selected)
