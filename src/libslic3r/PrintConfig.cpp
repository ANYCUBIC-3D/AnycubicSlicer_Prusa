...
static const t_config_enum_values s_keys_map_GCodeThumbnailsFormat = {
    ...
    { "PREPNG", int(GCodeThumbnailsFormat::PREPNG) },
...
void PrintConfigDef::init_common_params()
{
    ...
    def->mode = comExpert;
    def->set_enum<GCodeThumbnailsFormat>({ "PNG", "JPG", "QOI", "PREPNG" });
    ...
    def           = this->add("rect_size_x", coFloat);
    def->label    = L("Width (X Axis)");
    def->tooltip  = L("Size in X of the rectangular plate.");
    def->sidetext = L("mm");
    def->min      = 2;
    def->max      = 480;
    def->mode     = comSimple;
    ...
    def           = this->add("rect_size_y", coFloat);
    def->label    = L("Depth (Y Axis)");
    def->tooltip  = L("Size in Y of the rectangular plate.");
    def->sidetext = L("mm");
    def->min      = 2;
    def->max      = 480;
    def->mode     = comSimple;
...
void PrintConfigDef::init_fff_params()
{
    ...
    def->tooltip = L("This end procedure is inserted at the end of the output file. "
                   "Note that you can use placeholder variables for all AnycubicSlicer settings.");
    def->multiline = true;
    def->full_width = true;
    def->height = 12;
    def->mode = comExpert;
    ...
    def->set_enum_values(ConfigOptionDef::GUIType::select_close, {
        "PLA", 
        "PETG",
        "ABS",
        "ASA",
    ...
    def->tooltip = L("Some G/M-code commands, including temperature control and others, are not universal. "
                   ...
                   "The \"No extrusion\" flavor prevents AnycubicSlicer from exporting any extrusion value at all.");
    ...
    def->tooltip = L("Connect an infill line to an internal perimeter with a short segment of an additional perimeter. "
                     ...
                     "AnycubicSlicer tries to connect two close infill lines to a short perimeter segment. If no such perimeter segment "
    ...
    def->mode = comExpert;
//#if defined(_WIN32)
//    def->set_default_value(new ConfigOptionStrings{"./plugins/ArcWelder.exe"});
//#else
//    def->set_default_value(new ConfigOptionStrings{"./plugins/ArcWelder"});
//#endif


    def = this->add("post_process_enable", coBool);
...
//    def->gui_type = ConfigOptionDef::GUIType::slider;
    ...
    def->tooltip = L("This start procedure is inserted at the beginning, after bed has reached "
                   ...
                   "has finished heating. If AnycubicSlicer detects M104 or M190 in your custom codes, "
                   "such commands will not be prepended automatically so you're free to customize "
                   "the order of heating commands and other custom actions. Note that you can use "
                   "placeholder variables for all AnycubicSlicer settings, so you can put "
                   "a \"M109 S[first_layer_temperature]\" command wherever you want.");
    def = this->add("autoemit_temperature_commands", coBool);
    def->label = L("Emit temperature commands automatically");
    def->tooltip = L("When enabled, AnycubicSlicer will check whether your Custom Start G-Code contains M104 or M190. "
                     ...
                     "AnycubicSlicer will execute the Start G-Code after bed reached its target temperature "
                     "and extruder just started heating.\n\n"
                     "When disabled, AnycubicSlicer will NOT emit commands to heat up extruder and bed, "
    ...
    def->tooltip = L("This start procedure is inserted at the beginning, after any printer start gcode (and "
                   ...
                   "This is used to override settings for a specific filament. If AnycubicSlicer detects "
                   ...
                   "for all AnycubicSlicer settings, so you can put a \"M109 S[first_layer_temperature]\" command "
...
//    def->min = 0;
    ...
    def->tooltip = L("This custom code is inserted before every toolchange. Placeholder variables for all AnycubicSlicer settings "
                     "as well as {toolchange_z}, {previous_extruder} and {next_extruder} can be used. When a tool-changing command "
                     "which changes to the correct extruder is included (such as T{next_extruder}), AnycubicSlicer will emit no other such command. "
...
// Ignore the following obsolete configuration keys: //"clip_multipart_objects",
static std::set<std::string> PrintConfigDef_ignore = {
    ...
    // Introduced in some Anycubic Slicer 2.3.1 alpha, later renamed or removed.
    "fuzzy_skin_perimeter_mode", "fuzzy_skin_shape",
    // Introduced in Anycubic Slicer 2.3.0-alpha2, later replaced by automatic calculation based on extrusion width.
...
void PrintConfigDef::handle_legacy(t_config_option_key &opt_key, std::string &value)
{
    ...
    if (opt_key == "extrusion_width_ratio" || opt_key == "bottom_layer_speed_ratio"
        ...
        if (value == "makerbot")
            ...
            // the "new" marlin firmware flavor used to be called "marlinfirmware" for some time during Anycubic Slicer 2.4.0-alpha development.
            value = "marlin2";
    	...
    	// Anycubic Slicer no more accepts skirt_height == -1 to print a draft shield to the top of the highest object.
        // A new "draft_shield" enum config value is used instead.
    	opt_key = "draft_shield";
        value = "enabled";
    } else if (opt_key == "draft_shield" && (value == "1" || value == "0")) {
        // draft_shield used to be a bool, it was turned into an enum in Anycubic Slicer 2.4.0.
    ...
    // In Anycubic Slicer 2.3.0-alpha0 the "monotonous" infill was introduced, which was later renamed to "monotonic".
