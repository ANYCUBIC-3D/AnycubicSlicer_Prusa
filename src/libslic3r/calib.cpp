...
namespace Slic3r {
    ...
    std::string calib_pressure_advance::generate_test(double start_pa /*= 0*/, double step_pa /*= 0.002*/,
        ...
        Points      bedpts = get_bed_shape(mp_gcodegen->config());

        BoundingBox bed_ext  = Polygon(bedpts).bounding_box().inflated(scale_(-5));
    ...
    //}
      ...
      const float w = unscale_( bed_ext.size().x());
      const float h = unscale_(bed_ext.size().y());
