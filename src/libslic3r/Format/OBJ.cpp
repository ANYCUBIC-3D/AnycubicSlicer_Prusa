...
bool load_obj(std::istream &stream, TriangleMesh *meshptr) {
  if (meshptr == nullptr)
    return false;

  // Parse the OBJ file.
  ObjParser::ObjData data;
  if (!ObjParser::objparse(stream, data)) {
    BOOST_LOG_TRIVIAL(error) << "load_obj: failed to parse stream";
    return false;
  }
    ...
    if (size_t num_face_vertices = j - i; num_face_vertices > 0) {
      if (num_face_vertices > 4) {
        ...
        BOOST_LOG_TRIVIAL(error)
            << "load_obj: failed to parse stream"
  ...
  for (size_t i = 0; i < num_vertices; ++i) {
    size_t j = i << 2;
    its.vertices.emplace_back(data.coordinates[j], data.coordinates[j + 1],
                              data.coordinates[j + 2]);
  }
  int indices[4];
  for (size_t i = 0; i < data.vertices.size();)
    if (data.vertices[i].coordIdx == -1)
      ...
      int cnt = 0;
      while (i < data.vertices.size())
        if (const ObjParser::ObjVertex &vertex = data.vertices[i++];
            vertex.coordIdx == -1) {
          break;
        } else {
          assert(cnt < 4);
          if (vertex.coordIdx < 0 ||
              vertex.coordIdx >= int(its.vertices.size())) {
            BOOST_LOG_TRIVIAL(error)
                << "load_obj: failed to parse stream"
                << ". The file contains invalid vertex index.";
            return false;
          }
          indices[cnt++] = vertex.coordIdx;
        }
...
bool load_obj(std::istream &stream, Model *model, const char *object_name_in) {
  TriangleMesh mesh;

  bool ret = load_obj(stream, &mesh);

  if (ret) {
    boost::uuids::random_generator rg;
    std::string object_name = boost::uuids::to_string(rg());

    model->add_object(object_name.c_str(), object_name.c_str(),
                      std::move(mesh));
  }
