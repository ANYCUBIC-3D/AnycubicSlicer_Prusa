...
// Because of a crash in Anycubic Slicer 2.3.0/2.3.1 when showing an update notification with some locales, we don't want Anycubic Slicer 2.3.0/2.3.1
// to show this notification. On the other hand, we would like Anycubic Slicer 2.3.2 to show an update notification of the upcoming Anycubic Slicer 2.4.0.
...
std::string AppConfig::load(const std::string &path)
{
    ...
    try {
        ...
        if (!recovered) {
            // Report the initial error of parsing Anycubic Slicer.ini.
            // Error while parsing config file. We'll customize the error message and rethrow to be displayed.
            // ! But to avoid the use of _utf8 (related to use of wxWidgets) 
            // we will rethrow this exception from the place of load() call, if returned value wouldn't be empty
            /*
            throw Slic3r::RuntimeError(
                _utf8(L("Error parsing Anycubic Slicer config file, it is probably corrupted. "
    ...
    if (ini_ver) {
        ...
        m_legacy_datadir = false;//ini_ver < Semver(1, 40, 0);
    }
...
void AppConfig::save()
{
    ...
    // On Windows, the rename is likely NOT atomic, thus it may fail if Anycubic Slicer crashes on another thread in the meanwhile.
