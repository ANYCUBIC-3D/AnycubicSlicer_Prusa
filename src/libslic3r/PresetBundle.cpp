...
const char *PresetBundle::PRUSA_BUNDLE = "Anycubic";
    ...
    physical_printers(PhysicalPrinter::printer_options())
{
    ...
    int default_printer_size = 1;
    for (size_t i = 0; i < default_printer_size; ++ i) {
...
// Import newer configuration from alternate Anycubic Slicer configuration directory.
...
void PresetBundle::import_newer_configs(const std::string& from)
{
    ...
    // by the copied Anycubic Slicer.ini
    try {
...
void PresetBundle::load_installed_filaments(AppConfig &config)
{
    if (! config.has_section(AppConfig::SECTION_FILAMENTS)) {
		// Compatibility with the Anycubic Slicer 2.1.1 and older, where the filament profiles were not installable yet.
...
void PresetBundle::load_installed_sla_materials(AppConfig &config)
{
    if (! config.has_section(AppConfig::SECTION_MATERIALS)) {
		...
		// Compatibility with the Anycubic Slicer 2.1.1 and older, where the SLA material profiles were not installable yet.
...
ConfigSubstitutions PresetBundle::load_config_file(const std::string &path, ForwardCompatibilitySubstitutionRule compatibility_rule,std::vector<std::string> &infoList)
{
    if (is_gcode_file(path)) {
        DynamicPrintConfig config;
        config.apply(FullPrintConfig::defaults());
        std::vector<std::string> _infoList            = {infoList[6], infoList[7], infoList[8], infoList[9]};
        ConfigSubstitutions      config_substitutions = config.load_from_gcode_file(path, compatibility_rule, _infoList);
    ...
    try {
        ...
        throw Slic3r::RuntimeError(format(infoList[0], path, err.what()));
        ...
        throw Slic3r::RuntimeError(format(infoList[1], err.filename(), err.message(), err.line()));
    } catch (const std::runtime_error &err) {
        throw Slic3r::RuntimeError(format(infoList[2], path, err.what()));
    ...
    try {
        ...
        //case CONFIG_FILE_TYPE_UNKNOWN: throw Slic3r::RuntimeError(std::string("Unknown configuration file type: ") + path);
        case CONFIG_FILE_TYPE_UNKNOWN: throw Slic3r::RuntimeError(format(infoList[3], path));
        case CONFIG_FILE_TYPE_APP_CONFIG:
            throw Slic3r::RuntimeError(format(infoList[4], path));
        ...
        case CONFIG_FILE_TYPE_CONFIG_BUNDLE: return load_config_file_config_bundle(path, tree, compatibility_rule);
        }
    } catch (const ConfigurationError &e) {
        throw Slic3r::RuntimeError(format(infoList[5], path, e.what()));
    }
