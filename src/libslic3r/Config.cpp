...
std::vector<std::string> ConfigOptionDef::cli_args(const std::string &key) const
{
	...
	if (this->cli != ConfigOptionDef::nocli) {
        ...
        // Neither '=' nor '!' is used in any of the cli parameters currently defined by Anycubic Slicer.
...
void ConfigDef::finalize()
{
    ...
    for (std::pair<const t_config_option_key, ConfigOptionDef> &kvp : options) {
        ...
        if (def.opt_key == "filament_type" && def.gui_type == ConfigOptionDef::GUIType::select_close) {
            continue;
        }
        if (def.type == coEnum) {
            assert(def.enum_def);
...
bool ConfigBase::set_deserialize_raw(const t_config_option_key &opt_key_src, const std::string &value, ConfigSubstitutionContext& substitutions_ctxt, bool append)
{
    ...
    if (optdef->type == coBools && substitutions_ctxt.rule != ForwardCompatibilitySubstitutionRule::Disable) {
    	//FIXME Special handling of vectors of bools, quick and not so dirty solution before Anycubic Slicer 2.3.2 release.
...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Also the "; generated by Anycubic Slicer ..." comment line will be removed.
    size_t j = 0;
...
public:
    ...
    // Stop at file_start
    ReverseLineReader(std::istream &ifs, pos_type file_start) : m_ifs(ifs), m_file_start(file_start)
    {
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(bool isACLoad,const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule,std::vector<std::string> &infoList)
{
    // Read a 64k block from the end of the G-code.
    boost::nowide::ifstream ifs(file, std::ifstream::binary);
    // Look for Slic3r or Anycubic Slicer header.
    ...
    bool has_delimiters = false;
    {
        ...
        static constexpr const char prusaslicer_gcode_header[] = "; generated by AnycubicSlicer ";
        std::string                 header;
        ...
        if (!header_found)
            throw Slic3r::RuntimeError(infoList[0]);
        // throw Slic3r::RuntimeError("Not a AnycubicSlicer / Slic3r PE generated g-code.");
    }
    ...
    if (has_delimiters) {
        // Anycubic Slicer starting with 2.4.0-alpha0 delimits the config section stored into G-code with
        // ; Anycubic Slicer_config = begin
        // ...
        // ; Anycubic Slicer_config = end
        ...
        while (reader.getline(line))
            if (line == "; AnycubicSlicer_config = end") {
                end_found = true;
                break;
            }
        if (!end_found)
            throw Slic3r::RuntimeError(format(infoList[1], file));
        // throw Slic3r::RuntimeError(format("Configuration block closing tag \"; AnycubicSlicer_config = end\" not found when reading %1%", file));
        std::string key, value;
        while (reader.getline(line)) {
            if (line == "; AnycubicSlicer_config = begin") {
                begin_found = true;
                break;
            }
        ...
        if (!begin_found)
            throw Slic3r::RuntimeError(format(infoList[2], file));
        // throw Slic3r::RuntimeError(format("Configuration block opening tag \"; AnycubicSlicer_config = begin\" not found when reading
        // %1%", file));
    } else {
        // Slic3r or Anycubic Slicer older than 2.4.0-alpha0 do not emit any delimiter.
    ...
    if (key_value_pairs < 80)
        throw Slic3r::RuntimeError(format(infoList[3], file, key_value_pairs));
    // throw Slic3r::RuntimeError(format("Suspiciously low number of configuration values extracted from %1%: %2%", file, key_value_pairs));
    return std::move(substitutions_ctxt.substitutions);
}

ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule,std::vector<std::string> &infoList)
{
    // Read a 64k block from the end of the G-code.
    boost::nowide::ifstream ifs(file, std::ifstream::binary);
    // Look for Slic3r or Anycubic Slicer header.
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(bool isACLoad,std::function<void(double, double,std::string)> setPrintStatistics_callback,const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    // Read a 64k block from the end of the G-code.
    boost::nowide::ifstream ifs(file, std::ifstream::binary);
    // Look for Slic3r or Anycubic Slicer header.
    ...
    bool has_delimiters = false;
    {
        ...
        if (!header_found)
            throw Slic3r::RuntimeError("Not a Slic3r base generated g-code.");
    }
    ...
    if (has_delimiters) {
        ...
        if (!end_found)
            throw Slic3r::RuntimeError(
                format("Configuration block closing tag \"; AnycubicSlicer_config = end\" not found when reading %1%", file));
        std::string key, value;
        while (reader.getline(line)) {
            if (line == "; AnycubicSlicer_config = begin") {
                begin_found = true;
        ...
        if (!begin_found)
            throw Slic3r::RuntimeError(
                format("Configuration block opening tag \"; AnycubicSlicer_config = begin\" not found when reading %1%", file));
    } else {
        // Slic3r or Anycubic Slicer older than 2.4.0-alpha0 do not emit any delimiter.
        // Try a heuristics reading the G-code from back.
        ifs.seekg(0, ifs.end);
        ...
        key_value_pairs = load_from_gcode_string_legacy(*this, data.data(), substitutions_ctxt);
        if (isACLoad) {
            std::function<double(const char *, const char *str)> searchStrCallbackIndex = [](const char *searchStr, const char *str) {
                double      filamentCost;
                const char *costStart = strstr(str, searchStr);
                if (costStart != nullptr) {
                    costStart += strlen(searchStr);
                    const char *newline = strchr(costStart, '\n');
                    if (newline != nullptr) {
                        size_t valueLength = newline - costStart;
                        char * costValue   = new char[valueLength + 1];
                        strncpy(costValue, costStart, valueLength);
                        costValue[valueLength] = '\0';
                        filamentCost           = std::stod(costValue);
            ...
            std::function<std::string(const char *, const char *str)> searchStrCallbackIndex_str = [](const char *searchStr, const char *str) {
                std::string      filamentCost;
                const char *costStart = strstr(str, searchStr);
                if (costStart != nullptr) {
                    costStart += strlen(searchStr);
                    const char *newline = strchr(costStart, '\n');
            ...
            setPrintStatistics_callback(searchStrCallbackIndex("filament used [mm] = ", data.data()),
                                        searchStrCallbackIndex("filament used [g] = ", data.data()),
                                        searchStrCallbackIndex_str("estimated printing time (normal mode) = ", data.data()));
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    if (has_delimiters)
    {
        ...
        if (! begin_found) 
            throw Slic3r::RuntimeError(format("Configuration block opening tag \"; AnycubicSlicer_config = begin\" not found when reading %1%", file));
    }
    else
    {
        // Slic3r or Anycubic Slicer older than 2.4.0-alpha0 do not emit any delimiter.
...
// Load the config keys from the tail of a G-code stream.
ConfigSubstitutions ConfigBase::load_from_gcode_stream( std::istream &ifs, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    if (has_delimiters)
    {
        // AnycubicSlicer starting with 2.4.0-alpha0 delimits the config section stored into G-code with
        ...
        if (! end_found)
            throw Slic3r::RuntimeError("Configuration block closing tag \"; AnycubicSlicer_config = end\" not found when reading stream");
        std::string key, value;
        while (reader.getline(line)) {
            if (line == "; AnycubicSlicer_config = begin") {
                begin_found = true;
        ...
        if (! begin_found)
            throw Slic3r::RuntimeError("Configuration block opening tag \"; AnycubicSlicer_config = begin\" not found when reading stream");
    }
    else
    {
    ...
    if (key_value_pairs < 80)
        throw Slic3r::RuntimeError(format("Suspiciously low number of configuration values extracted from stream: %1%",  key_value_pairs));
    return std::move(substitutions_ctxt.substitutions);
}
