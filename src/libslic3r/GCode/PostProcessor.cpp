...
static int run_script(const std::string &script, const std::string &gcode, std::string &/*std_err*/)
{
    ...
    std::wstring wcScript = boost::nowide::widen(script);
    //const wchar_t* szArglist = wcScript.c_str();
  ...
  //  }
    ...
    std::wstring command = wcScript;
	if (! boost::filesystem::exists(boost::filesystem::path(command)))
		throw Slic3r::RuntimeError(std::string("The configured post-processing script does not exist: ") + script + std::string(".command:") + boost::nowide::narrow(command) );
